...
WebSocket over `ws://` for Phase 1 (localhost and LAN). `wss://` (TLS) is required for Phase 5 (Online Mode). The server must listen on a configurable port (default `2567`, the Colyseus conventional default).
