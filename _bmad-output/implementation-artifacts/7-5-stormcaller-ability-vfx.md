...
- [ ] **Task 2: `apps/host-client/src/vfx/ability-visuals.ts` — pure, PixiJS-free mapping layer (AC: 1, 2, 3, 5, 8)**
  ...
  - [ ] 2.2: **Update — Story 7.9 / ADR-0003:** the hit geometry now lives in the shared **ability presentation contract** in `shared-types`. **Import** `ABILITY_HIT_RANGE_PX`, `ABILITY_HIT_RADIUS_PX`, `ABILITY_DELIVERY`, and `STORM_EYE_ZONE_RADIUS_PX` from `shared-types` and derive the Stormcaller slices (`ABILITY_HIT_RANGE_PX.stormcaller` = `[160, 200, 0, 160]`, `ABILITY_HIT_RADIUS_PX.stormcaller` = `[60, 70, 110, 80]`, `STORM_EYE_ZONE_RADIUS_PX` = 150) from it — **do not** re-declare them as local literals (that is the transcription drift 7.9 removed). Only genuinely cosmetic constants stay local. (The original "declare locally, never import game-rules" instruction is superseded: the host still never imports `game-rules` *logic*, but the shared spatial contract in `shared-types` is importable.)
...
- [ ] **Task 3: `apps/host-client/src/vfx/stormcaller-vfx.ts` — thin primitive binders (AC: 1, 2, 3, 4)**
  ...
  - [ ] 3.4: `export function spawnStormEyeStrike(engine, x, y, now): void` — the optional `zone:strike` accent (Task 6). Pass `now` as the factory's `startedAt` (it is a delta-triggered effect — Task 6.3 hands it `Date.now()`).
