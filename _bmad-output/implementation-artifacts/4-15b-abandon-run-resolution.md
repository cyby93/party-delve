...
- [ ] **Task 1 — Vote-tracking field (AC: 1, 3)**
  - [ ] Add `private abandonVotes = new Map<string, 'accept'>();` immediately after `private runVotes` (`GameRoom.ts:132`). Value type is `'accept'` only — unlike `runVotes`, a decline never gets recorded because it cancels the whole proposal (AC4).
  - [ ] Extend the `net-protocol` type import at `GameRoom.ts:5` with `AbandonProposeMsg, AbandonVoteMsg`.

- [ ] **Task 2 — `run:abandon-propose` handler (AC: 1, 2)**
  - [ ] Register `this.onMessage(EventNames.RUN_ABANDON_PROPOSE, ...)` directly after the existing `EventNames.VOTE` handler (`GameRoom.ts:235-258`). Copy its `try/catch` + `typeof raw === 'string' ? JSON.parse(raw) : raw` shape even though the payload is empty — every handler in this file is defensive the same way.
  - [ ] Guards, in this order: `if (this.gameState.session.phase !== 'dungeon') return;` → `if (this.gameState.abandonProposal !== null) return;` → `const player = ...find(p => p.id === client.sessionId); if (!player || player.isFrozen) return;`
  - [ ] `this.abandonVotes.clear();` then `this.gameState.abandonProposal = { proposedBy: client.sessionId };`
  - [ ] Broadcast `EventNames.SNAPSHOT` (see Dev Notes → "Why a snapshot, not a delta, on propose").
  - [ ] `logger.info({ roomId, proposedBy }, 'abandon proposed')`.

- [ ] **Task 3 — `run:abandon-vote` handler (AC: 3, 4)**
  - [ ] Register `this.onMessage(EventNames.RUN_ABANDON_VOTE, ...)` after Task 2's handler.
  - [ ] Guards: `phase !== 'dungeon'` → `abandonProposal === null` → unknown/frozen player.
  - [ ] `if (!msg.accept) { this.cancelAbandonProposal('declined', client.sessionId); return; }`
  - [ ] Otherwise `this.abandonVotes.set(client.sessionId, 'accept');` then `this.resolveAbandonVoteIfComplete();`

- [ ] **Task 4 — Resolution + cancellation helpers (AC: 3, 4)**
  - [ ] Add `private resolveAbandonVoteIfComplete()` and `private cancelAbandonProposal(reason, byPlayerId?)` and `private abandonRun()` immediately after `resolveVoteIfComplete()` (`GameRoom.ts:632`). Exact bodies in Dev Notes → "Helper bodies".

- [ ] **Task 5 — Cancel on every leave path (AC: 4)**
  - [ ] `onLeave`, consented branch (`GameRoom.ts:464-490`): call `this.cancelAbandonProposal('player-left', client.sessionId);` just before the `return` at `:490`, after the `player:left` broadcast.
  - [ ] `onLeave`, network-drop branch: call it right beside the existing `this.resolveVoteIfComplete(); this.checkReturnReady();` pair (`:502-503`).
  - [ ] `onLeave`, grace-expiry branch: same, beside `:568-569`.
  - [ ] **Do not** call `resolveAbandonVoteIfComplete()` from any of these — see Dev Notes → "Disconnect cancels (divergence from the run-start vote)".

- [ ] **Task 6 — `resetToHub()` additions (AC: 5)**
  - [ ] After the boss-body / arena-wall destruction block (`GameRoom.ts:788-793`), add `this.gameState.boss = null;` and `this.pendingBossStompEvents.length = 0;`.
  - [ ] Beside `this.runVotes.clear();` (`:859`), add `this.gameState.abandonProposal = null;` and `this.abandonVotes.clear();`.
  - [ ] Do **not** change anything else in `resetToHub()` — in particular leave `player.isFrozen = false` (`:821`) alone; see Dev Notes → "Pre-existing quirks: do not fix here".

- [ ] **Task 7 — e2e tests (AC: 6)**
  - [ ] Create `tests/e2e/abandon-run.test.ts` with the four scenarios in Dev Notes → "Testing requirements".

- [ ] **Task 8 — Simulation-safety hook (AC: 7)**
  - [ ] `npm run typecheck` (root) — clean.
  - [ ] `npm test` (root) — green apart from the two known baseline failures.
  - [ ] Deterministic-tick + perf sanity checks per Dev Notes; record both in Completion Notes.
