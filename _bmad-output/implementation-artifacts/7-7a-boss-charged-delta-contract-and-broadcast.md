...
- [x] **Task 1: Read before writing — confirm the baseline claims** (AC: 1, 2, 3, 4, 6)
  - [x] 1.1: `packages/net-protocol/src/messages/server-to-host.ts` — read the whole union;
  ...
  - [x] 1.2: `packages/net-protocol/src/apply-delta.ts` — read `case 'boss:stomped'` (`:185-186`),
        `case 'enemy:stomped'` (`:131-132`), `case 'boss:moved'` (`:181-184`) and the
        `default:` exhaustiveness guard (`:289-294`).
  - [x] 1.3: `packages/game-rules/src/entities/grassland-boss.ts` — `BossChargedEvent` (`:15`),
        `tryCharge` (`:78-91`), the charge gate (`:186-189`). **Read-only: do not edit this file.**
  - [x] 1.4: `apps/simulation-server/src/rooms/GameRoom.ts:1888-1960` — the boss-event switch.
  - [x] 1.5: Verify nothing else must change:
...
- [x] **Task 2: Add `BossChargedDelta` to the protocol** (AC: 1)
  - [x] 2.1: In `server-to-host.ts`, add immediately after `BossStompedDelta` (`:189-195`):
        ```ts
        ...
        ```
  - [x] 2.2: Add `| BossChargedDelta` to the `DeltaEventMsg` union, positioned next to
        `BossStompedDelta` (`:320`) to keep the boss members grouped.
  - [x] 2.3: Add `BossChargedDelta` to the `export type { … }` list in
...
- [x] **Task 3: Add the explicit `applyDelta` no-op** (AC: 2, 3)
  - [x] 3.1: In `apply-delta.ts`, directly under `case 'boss:stomped'` (`:185-186`), add:
        ```ts
        case 'boss:charged':
        ...
        ```
  - [x] 3.2: Confirm the `default:` `never` guard now compiles (it will fail loudly if 3.1 was skipped).

- [x] **Task 4: Replace the swallow with a broadcast in `GameRoom`** (AC: 4, 5, 7)
  - [x] 4.1: Replace `GameRoom.ts:1926-1928` with:
        ```ts
        case 'boss:charged':
          ...
          break;
        ```
  - [x] 4.2: Do **not** add a `this.gameState.boss.position` write (unlike `case 'boss:moved'`
  ...
  - [x] 4.3: Confirm no `logger.*` call was added inside the tick path (AC7).

- [x] **Task 5: Contract test** (AC: 8)
  - [x] 5.1: Extend `tests/contract/net-protocol.test.ts`'s
        ...
        ```
  - [x] 5.2: Add the `applyDelta` no-op assertion in the same block (use the file's existing
        ...
        ```
  - [x] 5.3: Also assert the delta round-trips through the generic union
  ...
  - [x] 5.4: Run: `cd /home/cyby/projects/party-delve/tests && npx vitest run contract/net-protocol.test.ts`

- [x] **Task 6: Contract-change hook discharge** (AC: 9) — *triggered: `packages/net-protocol/**` changed*
  - [x] 6.1: **Protocol Architect review** — request explicit review of the diff to
        `server-to-host.ts`, `index.ts`, `apply-delta.ts` before merge. Record the outcome in
        the Dev Agent Record.
  - [x] 6.2: **Compatibility checklist** — walk and record each item of the checklist in
        Dev Notes → "Backward Compatibility Checklist". Do not tick items you did not verify.
  - [x] 6.3: **Spec update** — `docs/specs/networking-spec.md` does not enumerate
  ...
  - [x] 6.4: **≥1 contract test** — satisfied by Task 5.

- [x] **Task 7: Simulation-safety hook discharge** (AC: 9) — *triggered: `apps/simulation-server/**` changed*
  - [x] 7.1: **Typecheck (all 10 tsconfigs):** `cd /home/cyby/projects/party-delve && npm run typecheck`
        — must exit 0. This is also the AC2/AC6 exhaustiveness proof.
  - [x] 7.2: **Unit tests:** `cd /home/cyby/projects/party-delve/tests && npx vitest run unit`
        — the pure-logic suites. No behaviour change is expected; any failure means Task 4
        touched more than the broadcast.
  - [x] 7.3: **Simulation-server suites:**
        `cd /home/cyby/projects/party-delve/apps/simulation-server && npx vitest run`
        — 9 `game-room-*` / `physics-world` suites.
  - [x] 7.4: **Deterministic tick test:** there is **no dedicated boss-tick determinism suite**
  ...
  - [x] 7.5: **Replay test:** none exists in this repo (verified:
  ...
  - [x] 7.6: **Perf sanity:** record the AC7 quantification (≤1 broadcast per 150 ticks / 5 s,
...
- [x] **Task 8: Full-suite regression + honest flake accounting** (AC: 4, 9)
  - [x] 8.1: `cd /home/cyby/projects/party-delve && npm test` (root, full suite).
  - [x] 8.2: `tests/e2e` is **known-flaky under WSL2** and not caused by this story — a 60 s
...
- [x] **Task 9: Self-check before marking done** (AC: 1-9)
  - [x] 9.1: `cd /home/cyby/projects/party-delve && rtk git diff --stat` — the changed-file set
  ...
  - [x] 9.2: Confirm `GameRoom.ts`'s diff is the single `case 'boss:charged'` block and nothing else.
  - [x] 9.3: Confirm the completion notes state that **the delta is deliberately unconsumed**
        after this story (7.7b wires the host) and that **no windup/telegraph state was added**.
  - [x] 9.4: Confirm `_bmad-output/implementation-artifacts/sprint-status.yaml` was **not** edited
        by the story-creation step (the dev-story workflow updates it at its own point).

### Review Findings

- [x] [Review][Decision] Contract-change hook's "Protocol Architect review" was self-attested, not independently performed — **Resolved by user (2026-07-26):** the three-layer code-review pass (Blind Hunter, Edge Case Hunter, Acceptance Auditor — each an independent subagent, none sharing this session's conversation context) that just ran against this diff is accepted as satisfying the Contract-change hook's "Protocol Architect review" requirement. All three layers independently re-verified every AC against actual source rather than trusting the story's baseline claims. Merge Gate item considered discharged.
- [x] [Review][Patch] `apply-delta.ts`'s new `boss:charged` comment hardcodes a line number ("line 181") that will drift the moment the file changes above it [packages/net-protocol/src/apply-delta.ts:189] — fixed: now references `case 'boss:moved'` by name instead of a line number.
- [x] [Review][Defer] `GameRoom.ts`'s boss-event `switch (evt.type)` has no exhaustive/`never` guard, unlike `apply-delta.ts`'s switch over `DeltaEventMsg` — a future `BossEvent` variant could compile cleanly with no case here and be silently dropped (proven by `boss:charged` itself sitting in exactly that state pre-diff: case present, only a `break`, no broadcast, no compile error) [apps/simulation-server/src/rooms/GameRoom.ts:1919] — deferred, pre-existing structural gap not caused by this diff; adding the guard would touch code outside this story's one-case-block scope. Logged as `D-7.7a-B`.
- [x] [Review][Defer] Boss charge does not re-sync the planck physics body — already identified during dev implementation (not newly found by review) and already logged as `D-7.7a-A` in `deferred-work.md`; re-confirmed independently by the Blind Hunter layer. No new action needed.
...
- Baseline verification (Task 1): `rtk grep -rn "boss:charged" --include=*.ts .` → exactly 3
  hits, matching the story's claim: `grassland-boss.ts:15` (type decl), `grassland-boss.ts:90`
  (`tryCharge` emission), `GameRoom.ts:1942` (the swallow — line number shifted from the story's
  `:1926` due to intervening commits, but content matched exactly).
- Contract tests: `cd tests && npx vitest run contract/net-protocol.test.ts` → `PASS (101) FAIL (0)`.
- Typecheck: `npm run typecheck` (all 10 tsconfigs) → exit 0, no errors. This is the compile-time
  proof that the `apply-delta.ts` exhaustiveness guard (`const _exhaustive: never = evt`) still
  holds with `BossChargedDelta` added to the union.
- Unit tests: `cd tests && npx vitest run unit` → `PASS (209) FAIL (0)`.
- Simulation-server suites: `cd apps/simulation-server && npx vitest run` → `PASS (81) FAIL (0)`.
- Full root suite (`npm test`), first run: `4 failed | 45 passed (49)` test files,
  `2 failed | 558 passed | 3 skipped (563)` tests. Failures:
  1. `tests/e2e/ability-dispatch.test.ts` — AC1 heal assertion at `:227`
     (`expected 35 to be greater than or equal to 100`) — the exact pre-existing flake documented
     in Story 7.1's Debug Log.
  2. `tests/e2e/full-run.test.ts` and `tests/e2e/hub-ability-use.test.ts` — both failed with
     "simulation-server did not start within 60s" (`tests/helpers/server.ts:37`) — the other
     documented WSL2 boot-timeout flake.
  3. `apps/host-client/src/vfx/ability-vfx.test.ts` — Stone Wall/Tremor Stomp/Iron Skin geometry
     assertion — a pre-existing failure in a file this story never touches (`host-client` is a
     blocked path here).
  Two-strike re-run of `tests/e2e` in isolation: `cd tests && npx vitest run e2e` →
  `PASS (8) FAIL (0) skipped (2)` — all e2e tests passed on the second run. The failure set
  *changed* between the two runs (full-suite run vs. isolated run), which per the story's Task 8.2
  and the established two-strike rule confirms this is the known WSL2 flake, not a regression
  introduced by this story. Not chased further, not claimed as passing on the first run.
- No `packages/game-rules/**` file was read-modified (read-only per the story's blocked paths);
  confirmed via `git diff --stat` (below) showing no entry under `packages/game-rules/`.

### Completion Notes List

- **AC1–AC2**: `BossChargedDelta = { type: 'boss:charged'; bossId: string; x: number; y: number }`
  added to `server-to-host.ts`, positioned immediately after `BossStompedDelta`, added to the
  `DeltaEventMsg` union, and exported from `net-protocol/src/index.ts`. No `radius` field, matching
  the internal `BossChargedEvent` exactly (verified field-for-field against
  `grassland-boss.ts:15`).
- **AC2–AC3**: `apply-delta.ts` gained an explicit `case 'boss:charged': return state;` no-op with
  a `ponytail:` comment explaining *why* — `boss:moved` remains the single writer of
  `state.boss.position`. The `default:`/`const _exhaustive: never = evt` guard compiles, proving
  the union member does not silently fall through.
- **AC3 (convergence)**: because `tryCharge` returns early from `tickBoss`, no `boss:moved` fires
  on the charge tick, so the host mirror lags by ≤ 11.7 px (`350 px/s ÷ 30 Hz`) against a 48 px
  boss for at most one 33 ms tick, corrected by the next tick's `boss:moved` or the periodic
  snapshot. This is the verified, not assumed, behaviour — recorded per AC3's requirement.
- **AC4**: `GameRoom.ts`'s `case 'boss:charged'` (previously a bare `break;` that swallowed the
  event) now broadcasts `{ type: 'boss:charged', bossId, x, y } satisfies DeltaEventMsg`,
  structurally matching the neighbouring `boss:moved`/`boss:stomped` blocks. No other line of
  `tickBoss`, the boss FSM, cooldowns, phases, damage, or physics-body handling was touched —
  confirmed by the diff being exactly this one case block (see Debug Log / `git diff --stat`).
- **AC5**: No sim-side windup/telegraph FSM state was added. The charge event remains strictly
  post-hoc — `tryCharge` moves the boss and emits the event in the same tick.
- **AC6 (graceful degradation)**: verified, not assumed — `applyDelta`'s `default:` branch returns
  `state` unchanged for any unknown type (no throw); `host-session.ts:44-63`'s whitelist is a
  positive OR-chain so an unrecognized `boss:charged` on an old host build is simply never
  forwarded to `onTransientDelta`, while `applyDelta` still runs unconditionally and no-ops; the
  mobile `App.tsx` delta handling is an `if`/`else if` chain with no exhaustive switch (confirmed:
  zero `'boss:'` references in `App.tsx`), so an unhandled type is silently ignored; both
  `host-session.ts` and `mobile-session.ts` wrap delta decode in `try {...} catch { /* ignore
  malformed delta */ }`. The only exhaustive switch over `DeltaEventMsg` in the repo is
  `apply-delta.ts`, which AC2 already fixes; every `satisfies DeltaEventMsg` site (GameRoom.ts
  broadcast literals, the contract test) only widens, so none could break.
- **AC7 (tick-loop hygiene)**: `tryCharge` fires at most once per `BOSS_CHARGE_COOLDOWN_TICKS = 150`
  ticks (5 s) — ≤ 12 broadcasts per 60 s boss fight, gated to `phase !== Phase1 && difficulty !==
  EASY`. The added `GameRoom.ts` code introduces no `logger.*` call, no `JSON` work, and allocates
  exactly one object literal per charge, matching `boss:stomped`/`boss:moved`. No profiling run was
  performed — none is warranted for a bounded, cooldown-gated single broadcast; this is stated
  explicitly rather than fabricated.
- **AC8**: contract tests added to the existing "Story 6.1 boss delta round-trips" block in
  `tests/contract/net-protocol.test.ts`: a named-type `BossChargedDelta` round-trip, a
  `satisfies DeltaEventMsg` union round-trip, and an `applyDelta` no-op assertion (same-reference
  `toBe(state)` plus an explicit check that a boss's `position` is unchanged after receiving a
  `boss:charged` delta with different coordinates). All pass (`PASS (101) FAIL (0)`).
- **AC9 — Contract-change hook discharge:**
  - *Protocol Architect review*: not performed by a separate reviewer agent in this pass — this
    single-agent `gds-dev-story` run has no second reviewer role to invoke mid-workflow. The
    change was self-verified line-by-line against the story's explicit field/type/case
    specifications (Tasks 1–4) before being written, and the cross-context work itself was
    pre-approved by the user on 2026-07-22 (recorded in the story's Pre-Task Hook). Flagging this
    as the honest gap, consistent with how Stories 3.18 and 3.2 recorded the same limitation: a
    real Protocol Architect review (or the `gds-code-review` pass this workflow triggers on
    completion) should still happen before merge.
  - *Compatibility checklist*: all 10 items in the story's Dev Notes → "Backward Compatibility
    Checklist" table were independently re-verified against current code (not just trusted from
    the story's baseline findings) — see Debug Log and AC6 note above. All 10 hold as stated;
    none contradicted current source.
  - *Spec update*: added a one-line `boss:charged` entry to `docs/specs/networking-spec.md`'s
    Simulation Events list. No ADR — this is an additive union member, not an authority/session/
    reconnect change.
  - *Contract test*: ≥1 added (three, in fact) — see AC8 above.
- **AC9 — Simulation-safety hook discharge:**
  - *Typecheck*: `npm run typecheck` (all 10 tsconfigs) exits 0.
  - *Unit tests*: `cd tests && npx vitest run unit` → `PASS (209) FAIL (0)`.
  - *Simulation-server suites*: `cd apps/simulation-server && npx vitest run` → `PASS (81) FAIL (0)`.
  - *Deterministic tick test*: no dedicated boss-tick determinism suite exists in this repo
    (verified: 0 `*boss*` files under `apps/simulation-server/tests`). Discharged by reasoning:
    this story touches zero `packages/game-rules/**` code, adds zero reads/writes of `boss` state
    beyond the pre-existing broadcast, consumes no PRNG, and adds no branch conditions —
    `tickBoss`'s event sequence for a given seed is unchanged.
  - *Replay test*: none exists in this repo (verified: 0 real hits for `rtk grep -rn "replay"
    tests/ apps/simulation-server/` — the 2 hits found are an unrelated code comment about vote
    replay). Recorded as "no replay harness exists — N/A" rather than omitted.
  - *Perf sanity*: recorded under AC7 above — no profiling run performed; none warranted.
- **Handoff to Story 7.7b** (per the story's Dev Notes → "Handoff contract to Story 7.7b"): the
  delta is broadcast but **deliberately unconsumed** after this story — no host code references
  `boss:charged` yet, matching Story 7.1's precedent of shipping a deliberately unwired artifact.
  7.7b must (1) add `delta.type === 'boss:charged'` to the `host-session.ts:44-63` whitelist
  (host-local filtering, not a contract change), (2) treat `x`/`y` as the post-charge position and
  render a post-hoc dash/impact, not a telegraph — there is no windup state and none was added,
  (3) size the visual against ≤ ~11.7 px of travel, deriving direction host-side if needed rather
  than requesting a protocol change, and (4) be aware `latestTransientDelta` in `App.tsx` is a
  single value cleared after 400 ms, so a charge co-arriving with e.g. `boss:damaged` in the same
  window can be dropped — a pre-existing limitation, not something to fix here.
- **Adjacent issue found, deliberately out of scope**: per the story's Dev Notes, `case
  'boss:moved'` re-syncs `this.bossBody` via `setPosition`, but the charge path does not, so
  `gameState.boss.position` and the physics body can disagree by ≤ ~11.7 px for up to one tick.
  Not fixed here (would be a boss-behaviour/physics change requiring its own Simulation-safety
  review). Logged as `D-7.7a-A` in `_bmad-output/implementation-artifacts/deferred-work.md`.
- **Confidence: 92%.** All 9 ACs verified against the actual current codebase (not just trusted
  from the story's baseline claims — every referenced file/line was independently re-read). The
  only reason this isn't higher: AC9's "Protocol Architect review" is self-attested rather than
  performed by an independent reviewer, per the honest gap noted above.

### File List

- `packages/net-protocol/src/messages/server-to-host.ts` — added `BossChargedDelta` type and
  union member
- `packages/net-protocol/src/index.ts` — exported `BossChargedDelta`
- `packages/net-protocol/src/apply-delta.ts` — added `case 'boss:charged'` no-op
- `apps/simulation-server/src/rooms/GameRoom.ts` — replaced the swallow with a broadcast
- `tests/contract/net-protocol.test.ts` — added `BossChargedDelta` round-trip, union round-trip,
  and `applyDelta` no-op tests
- `docs/specs/networking-spec.md` — added `boss:charged` line under Simulation Events
- `_bmad-output/implementation-artifacts/deferred-work.md` — added `D-7.7a-A` entry (physics-body
  desync, deliberately out of scope)
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — status transitions
  (`ready-for-dev` → `in-progress` → `review`), updated by the dev-story workflow itself
- `_bmad-output/implementation-artifacts/7-7a-boss-charged-delta-contract-and-broadcast.md` —
  this story file (task checkboxes, Dev Agent Record, Status)
...
| 2026-07-26 | Implemented: `BossChargedDelta` added to `net-protocol` (type, union member, export); explicit `applyDelta` no-op case; `GameRoom.ts`'s swallow replaced with a broadcast; 3 new contract tests (named-type round-trip, union round-trip, no-op assertion); `docs/specs/networking-spec.md` updated; Contract-change and Simulation-safety hooks discharged (typecheck, unit, sim-server, contract suites all green); `D-7.7a-A` (boss/physics-body desync) logged to `deferred-work.md`. Two pre-existing `tests/e2e` flakes and one pre-existing `host-client` geometry-test failure observed and confirmed unrelated (two-strike rule applied). Status → review. |
| 2026-07-26 | Code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, run against the uncommitted diff): 1 decision, 1 patch, 2 defer, 8 dismissed. User resolved the decision item — accepted this three-layer review as satisfying the Contract-change hook's "Protocol Architect review" requirement. Patch applied: removed a hardcoded line-number reference from the `apply-delta.ts` `boss:charged` comment. New defer `D-7.7a-B` logged (`GameRoom.ts`'s boss-event switch lacks an exhaustiveness guard — pre-existing, not introduced by this story). Status → done. |
