...
- [x] **Task 1 — One-time `VfxEngine` wiring in `DungeonScreen.tsx` (AC: 1, 6) — GUARDED: "if not already present".**
  ...
  - [x] 1.1: `import { VfxEngine, createParticleBurst, createRingShockwave, createBeam } from '../vfx';` (a plain relative import — no vite alias needed, per 7.1's Project Structure Notes).
  - [x] 1.2: `const vfxEngineRef = useRef<VfxEngine | null>(null);` alongside the other refs (`DungeonScreen.tsx:369-392`).
  - [x] 1.3: In `initPixi`, after `pixiAppRef.current = app;` (`:417`) and before `app.ticker.add(...)`: `vfxEngineRef.current = new VfxEngine(app.stage);`.
  - [x] 1.4: Inside the existing ticker callback, **after** the `renderFrame(...)` call (`:421-433`) and before the boss-sprite block: `vfxEngineRef.current?.update(Date.now());` — **exactly one call per frame, using `Date.now()`** to match `renderFrame`'s `const now = Date.now()` (`:99`). See the CLOCK CONTRACT pitfall in Dev Notes; do **not** use `performance.now()` even though the purification-pulse and reward-particle blocks below it do.
  - [x] 1.5: In the unmount cleanup (`:502-525`), before `app.destroy(...)`: `vfxEngineRef.current?.clear(); vfxEngineRef.current = null;`. Also clear the new refs added in Task 2.

- [x] **Task 2 — New host-side VFX modules (AC: 1, 2, 3, 4).**
...
- [x] **Task 3 — Pure, testable mapping functions (AC: 1, 2, 3).** All in `souldrinker-vfx.ts`, all synchronous, no clock reads, no PixiJS, no `packages/game-rules` import.
  - [x] 3.1: `planSouldrinkerCast(input: CastInput): VfxSpec[]` where `CastInput = { abilityIndex: number; casterX: number; casterY: number; dirX: number; dirY: number; hpFraction: number }` and `VfxSpec` is a discriminated union `{ kind: 'ring' | 'beam' | 'burst'; ...params }`. Returns `[]` for an unknown `abilityIndex`, for a zero-length direction vector, and for a non-finite input — never throws. **The zero-direction `[]` is correct for every Souldrinker index and must render nothing (no fallback flash):** all four abilities are AUTO/RELEASE/projectile with a non-zero hit range or projectile launch (none is a self-centred TAP), so the sim skips each on a zero direction (`GameRoom.ts:2074` projectile guard, `:2203` hitscan guard, `handleDarkPact` early-returns) — the host must not draw an effect the sim did not resolve. This mirrors Story 7.2's Avalanche resolution (code review 2026-07-22, Decision 2: *if the sim skips the hit, the host renders nothing*). Unlike Spiritcaller (7.3) and Stonehide (7.2), Souldrinker has no ability that needs the `focus === origin` exception.
  - [x] 3.2: `planBloodSpikeImpact(input: { hitX; hitY; casterX; casterY }): VfxSpec[]` — the lifesteal return beam + caster implode ring.
  - [x] 3.3: `planVoidPulseImpact(input: { hitX; hitY }): VfxSpec[]` — the void implode ring.
  - [x] 3.4: `planDamageBuffOnset(input: { x; y }): VfxSpec[]` — the buff-gained implode + void burst.
  - [x] 3.5: `classifyHpChanges(prev: ReadonlyMap<string, number>, next: readonly { id: string; hp: number }[]): { playerId: string; direction: 'gain' | 'loss'; amount: number }[]` — the AC3 classifier. Players absent from `prev` (join/reconnect) yield **no** change. Equal HP yields no change. This is the *only* HP-direction logic in the story; it decides loss-vs-gain purely from the sign, importing nothing.
  - [x] 3.6: `planHpLossCue(input: { x; y })` / `planHpGainCue(input: { x; y })` — the two opposed caster cues (dark outward droplets vs bright inward ring), used by both Blood Spike's self-cost and Dark Pact's drained ally.

- [x] **Task 4 — Blood Spike (AC: 1, 2).**
  - [x] 4.1: In the transient-delta `useEffect` (`:537-623`), extend the `ability:fired` branch. Look up the caster via `gameState?.players.find(p => p.id === delta.playerId)`.
...
- [x] **Task 6 — Dark Pact (AC: 1, 3).**
  ...
  - [x] 6.3: **Cost cue, classifier-driven.** New `useEffect` on `[gameState]` holding `prevPlayerHpRef = useRef<Map<string, number>>(new Map())`. Each run, call `classifyHpChanges(prevPlayerHpRef.current, gameState.players)`; for each result whose player `class === PlayerClass.SOULDRINKER` **or** who lost HP while a Souldrinker's Dark Pact cone was recently fired, spawn the loss cue at that player's position; then overwrite the ref. **Scope guard:** to keep out of 7.2/7.3/7.6's lane, apply the *gain* cue only to Souldrinkers (Spiritcaller heal visuals are 7.3's) and the *loss* cue only within `DARK_PACT_COST_CUE_WINDOW_MS = 400` of a Souldrinker `ability:fired` with `abilityIndex === 2` (tracked in a ref set by Task 6.1). Every other HP drop — bond drain, enemy melee, Blood Spike's own self-cost, which is already covered by Task 4.2 — is deliberately left untouched by this story.

- [x] **Task 7 — Void Pulse (AC: 1, 4).**
  - [x] 7.1: `abilityIndex === 3` → cast visual at the caster: an outward void ring sized to the ability's own hit radius (80) plus a slow inward-reading void particle burst.
  - [x] 7.2: In the `projectile:hit` branch from Task 4.5, Souldrinker slot 3 → `planVoidPulseImpact({ hitX, hitY })`: a void ring **imploding from radius 150 to 20** — 150 is exactly the chained pull zone's radius (`balance.ts:148`), so the impact visually announces the pull field that is about to appear.
  - [x] 7.3: Write the pull-zone appearance into `ability-vfx-config.ts` (Task 2.1) **as spec only**. Do not render it — the Zones block of `renderFrame` is 7.8's.

- [x] **Task 8 — Self-check (non-negotiable per project convention).** (8.1/8.2 automated; 8.3 manual Client-UX pass performed by Cyby 2026-07-24 — see below)
  - [x] 8.1: One test file, `apps/host-client/src/vfx/souldrinker-vfx.test.ts`, run with `npx vitest run src/vfx/souldrinker-vfx.test.ts` from `apps/host-client`. Cover: (a) each of the four `abilityIndex` values produces a **non-empty and mutually distinct** spec list from `planSouldrinkerCast` (assert distinctness by comparing serialized specs pairwise — this is AC1's machine-checkable core); (b) zero-length direction and non-finite inputs return `[]` without throwing; (c) Crimson Lash's ring centre equals `caster + dir * 180` and its radius is 65, and its beam width increases monotonically as `hpFraction` falls; (d) Void Pulse's impact ring has `startRadius: 150 > maxRadius`, i.e. it implodes; (e) `classifyHpChanges` returns `'loss'` on a decrease, `'gain'` on an increase, nothing on equality, and nothing for a player absent from `prev`.
  - [x] 8.2: `npm run typecheck` at repo root (covers all 10 tsconfigs).
  - [x] 8.3: Client-UX hook manual pass — **PERFORMED by Cyby (user) on 2026-07-24** on a live host + joined phone + simulation-server couch session. User reported all checklist points pass: (a) the four abilities read as visually distinct (Blood Spike / Crimson Lash / Dark Pact / Void Pulse); (b) Blood Spike's lifesteal return reads on the caster; (c) Dark Pact's cost cue (dark outward droplets) is not confusable with the buff-gained cue (bright void-purple inward ring); (d) couch readability at 2–4 m is legible; (e) the new effects do not occlude player/enemy bodies, health bars, status badges or the revive overlay; (f) Void Pulse's `0x7d2dff` reads as distinct from the boss's own purple disc at the boss level. Recorded on the user's verbal sign-off — Claude did not run this pass.
...
- **Never import `packages/game-rules` in `apps/host-client`.** Every balance number in this file (10 HP cost, 50% lifesteal, 180/65, 150, 0.25/4000 ms) is an *authoring-time* reference for picking visual parameters. Where a value must exist in host code it becomes a **local visual constant** in the new vfx module. `shared-types` **is** importable and already used (`PlayerClass`, `CLASS_DEFINITIONS`, `BossPhase`, `SessionColor` at `DungeonScreen.tsx:3-4`).
  - **Update — Story 7.9 / ADR-0003 (import, do not transcribe the geometry):** ability *spatial* geometry now lives in the shared **ability presentation contract** in `shared-types` and must be **imported from `shared-types`**, not copied. For this story that means the Void Pulse chained-zone radius is `VOID_PULSE_ZONE_RADIUS_PX` (import it — do not write a local `150`) and delivery type comes from `ABILITY_DELIVERY`; hit range/radius from `ABILITY_HIT_RANGE_PX`/`ABILITY_HIT_RADIUS_PX`. The rule is refined to *never import `game-rules` **logic***; pure balance (self-cost, lifesteal, HP-scaled damage, `DARK_PACT_DRAIN_PCT`, cooldowns) stays in `game-rules`, host-forbidden, and remains an authoring-time reference only.
...
- `npx vitest run src/vfx/souldrinker-vfx.test.ts` (from `apps/host-client`) → **9 passed / 0 failed**.
- `npm run typecheck` (repo root, all 10 tsconfigs) → **exit 0**.
- Full `apps/host-client` suite → 51 passed, **1 pre-existing failure** in `src/vfx/ability-vfx.test.ts:69` (7.2's Stonehide placement test). Verified pre-existing by stashing all of 7.4's tracked edits and re-running: fails identically. Root cause is Story 7.9's relocation of `ABILITY_HIT_RANGE_PX.stonehide[0]` to `160` (the test still asserts the Stonehide TAP slots hit at the caster, i.e. range `0`). It lives in Stonehide's (7.2) owned files, outside 7.4's scope/ownership — **not** caused by and **not** fixed by this story. Flagged as a cross-story drift follow-up.

### Completion Notes List

**Implemented (Tasks 1–7, 8.1, 8.2):**
- **Task 1 — SKIPPED (guarded, already present).** `vfxEngineRef` was already wired by 7.2/7.3 (`DungeonScreen.tsx` engine construction in `initPixi`, one `vfxEngineRef.current?.update(Date.now())` per ticker frame after `renderFrame`, teardown in unmount). Reused it verbatim — did not create a second engine. Confirmed the single `update(Date.now())` call and the `Date.now()` clock (CLOCK CONTRACT).
- **7.8 seam (`ability-vfx-config.ts`)** — data-only, **no PixiJS import**. Exports `SOULDRINKER_PALETTE` (the single palette source of truth, re-exported by `souldrinker-vfx.ts`), `PROJECTILE_APPEARANCE` (class-generic `Partial<Record<PlayerClass, [4]>>`, Souldrinker slots 0/3 populated, 1/2 `null`) and `ZONE_APPEARANCE` (keyed by `effectType: 'pull'`, with the derive-identity-from-`ownerId`+`effectType` rule documented). Renders nothing; the `renderFrame` Projectiles/Zones blocks are untouched (line-308 circle and the whole Zones block are byte-identical).
- **Pure planners (`souldrinker-vfx.ts`)** — `planSouldrinkerCast` returns a serializable `VfxSpec[]` (discriminated union `ring|beam|burst`), `[]` on unknown/non-integer index, non-finite input, or zero-aim (SILENT rule — sim skips every Souldrinker ability on zero aim). Plus `planBloodSpikeImpact` (return beam + implode ring, ring guaranteed **last** so the per-player bound can cancel it), `planBloodSpikeSplash`, `planVoidPulseImpact`, `planDamageBuffOnset`, `planHpLossCue`, `planHpGainCue`, and the pure `classifyHpChanges`. Geometry imported live from `shared-types` (`ABILITY_HIT_RANGE_PX`/`ABILITY_HIT_RADIUS_PX`/`VOID_PULSE_ZONE_RADIUS_PX`, Story 7.9) — no transcribed `150`/`180`/`65`/`80`. `spawnSouldrinkerVfx(engine, specs, startedAt)` is the only PixiJS-touching function; it returns ids in spec order and stamps `startedAt` into every factory (BACKGROUNDED-TICKER rule).
- **Suppress-and-replace (AC1, AC6).** New `else if (caster.class === SOULDRINKER)` branch in the `ability:fired` handler: never sets `entry.flashUntil` for any Souldrinker cast. Successful cast → per-ability VFX only; zero-aim skip → `[]` → nothing (no VFX, no flash). Non-Souldrinker classes keep the legacy flash path unchanged.
- **Blood Spike lifesteal (AC2).** `projectile:hit` added to the `host-session.ts` whitelist (one entry; host-local delivery only). `projectileMetaRef` (bundled into the existing `VfxContext` object — no 10th positional param) caches `class/abilityIndex/ownerId` at create-on-first-seen in the Projectiles block and is deleted alongside the Graphics; it survives `applyDelta` removing the projectile from state. Impact renders the return beam + inward-collapsing blood ring only when the owner is present/alive/not-spirit (sim skips the heal otherwise), else just the splash. Implode bounded to one live effect per player via `lifestealEffectIdRef` + `engine.remove(prevId)` (D-7.1-D mitigation).
- **Dark Pact (AC3).** Cast cone always renders. Buff-gained cue is snapshot-driven off `damageBuff` in `player.statusEffects` (new `[gameState]` effect + `buffedPlayersRef` onset diff). Cost cue is classifier-driven (new `[gameState]` effect + `prevPlayerHpRef`), gated to a 400 ms window after a Souldrinker Dark Pact `ability:fired` (`lastDarkPactCastAtRef`) so no other HP change (enemy melee, bond drain, Blood Spike self-cost/lifesteal) is touched. Cost cue = dark-red outward droplets; buff cue = void-purple inward ring — opposed on color/direction/shape.
- **Void Pulse (AC4).** Cast ring + slow burst at the caster; impact ring implodes from the chained-zone radius (150) to 20. Projectile/zone bodies are spec-only in `ability-vfx-config.ts`.

**Design decisions / notes for review:**
- During a successful Dark Pact both `[gameState]` effects fire on the caster: the void buff-onset (Task 6.2) **and** a blood HP-gain cue (Task 6.3, the transfer gain, gated to Souldrinkers + window). They read as "took HP back" + "got stronger" — intentional and spec-faithful (Task 3.6 requires the gain cue exist and be applied only to Souldrinkers). If a reviewer judges it too busy, suppressing the gain cue when the same player also received the buff is a one-line follow-up.
- `renderFrame` signature unchanged: `projectileMeta` was added to the existing `VfxContext` object (Dev Notes explicitly preferred bundling over a 10th positional Map param).
- Inherits the pre-existing single-value `latestTransientDelta` batching limitation (a `projectile:hit` colliding with another whitelisted delta in one batch drops that shot's lifesteal cue; the damage number still renders). Per Dev Notes, not rewritten here — raise an `App.tsx` delta queue as a follow-up if the drop rate breaks AC2 in the manual pass.

**Effect-volume observation (for 7.6/7.8, D-7.1-D):** every duration is ≤ 340 ms and no effect loops. Worst case is a 4-Souldrinker Blood-Spike party (~4 casts/s + ~4 hits/s), each producing ≤ 2 short-lived handles, so ~12 concurrent handles — matches the story's own estimate. Not measured against a live frame budget (see Task 8.3).

**Confidence: 80%.** Code paths are typecheck-clean and the pure planner/classifier logic is unit-tested against the ACs. The 20% gap is entirely the **unrun Task 8.3 manual Client-UX pass** — rendering fidelity (couch readability at 2–4 m, cost-vs-buff non-confusability, non-occlusion of bodies/HP bars/badges/revive overlay, and Void Pulse `0x7d2dff` vs the boss's own `0x7d2dff` disc at the boss level) cannot be verified without a live host + phone + simulation-server session, which this environment does not provide. **This must be performed by a human before merge.**

### File List

- `apps/host-client/src/vfx/souldrinker-vfx.ts` (new) — pure planners, `classifyHpChanges`, `spawnSouldrinkerVfx` executor, palette re-exports.
- `apps/host-client/src/vfx/ability-vfx-config.ts` (new) — the 7.8 projectile/zone appearance seam + `SOULDRINKER_PALETTE` (PixiJS-free).
- `apps/host-client/src/vfx/souldrinker-vfx.test.ts` (new) — 9 unit tests (AC1 distinctness, zero/non-finite guards, Crimson Lash geometry + HP scaling, Void Pulse implode, `classifyHpChanges`).
- `apps/host-client/src/vfx/index.ts` (modified) — barrel exports for the two new modules.
- `apps/host-client/src/screens/DungeonScreen.tsx` (modified) — Souldrinker `ability:fired` branch (suppress-and-replace), `projectile:hit` branch, `projectileMeta` on `VfxContext` + the Projectiles-block cache set/delete, buff-onset and cost/gain `[gameState]` effects, new refs + unmount cleanup.
- `apps/host-client/src/session/host-session.ts` (modified) — one line: `projectile:hit` added to the transient-delta whitelist (AC5).

### Review Findings

_Code review 2026-07-23 (gds-code-review, 3 parallel layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor). Acceptance Auditor: all six ACs MET. 5 findings dismissed as noise/unreachable/false-positive._

- [x] [Review][Patch] (APPLIED 2026-07-24) Dark Pact loss cue fires on the Souldrinker's own self-cost (resolved from Decision) — The cost/gain classifier effect (`DungeonScreen.tsx:1088-1107`) gates the cue on a single global 400 ms timestamp plus HP-sign only, so within the window a Souldrinker's own HP drop (Blood Spike's 10-HP self-cost, already shown by Task 4.2) draws a spurious dark-droplet loss cue, and the Dark Pact caster — who *gains* HP — could too. **Resolved 2026-07-23 (user decision, option 3):** gate the loss cue to `player.class !== PlayerClass.SOULDRINKER` (the drained ally), leaving the gain cue gated to Souldrinkers. Loss = non-Souldrinker allies, gain = Souldrinkers → mutually exclusive, and the Blood-Spike self-cost double-draw is eliminated. The broader coincidental-damage misfire (enemy melee / bond drain on a non-Souldrinker ally in the window) is knowingly accepted as-is — the drain delta carries no target, which is why the window was chosen.
- [x] [Review][Patch] (APPLIED 2026-07-24) Zero-aim Dark Pact still arms the correlation window [apps/host-client/src/screens/DungeonScreen.tsx:896] — `if (d.abilityIndex === 2) lastDarkPactCastAtRef.current = triggeredAt;` runs unconditionally, even when `planSouldrinkerCast(...)` returned `[]` for a zero-aim cast the sim skipped. The 400 ms cue window then opens for a cast that drained no one, mislabeling any coincident HP change. Fix: arm the window only when the aim is non-zero / the plan is non-empty.
- [x] [Review][Defer] Transient-delta batch-collapse drops simultaneous `projectile:hit` / `ability:fired` [apps/host-client/src/screens/DungeonScreen.tsx:1083, apps/host-client/src/App.tsx:20] — deferred, pre-existing. Single-slot `latestTransientDelta` useState keeps only the last delta of a synchronous flush; two projectile hits (or two casts) in one batch render only one, and projectile:hit has no `renderFrame` backstop (unlike cast:cancelled). Already disclosed in Dev Notes (line 394); the real fix is an App.tsx delta queue — cross-cutting, out of this story's scope.
- [x] [Review][Defer] `projectileMeta` miss for a projectile never seen in a delivered snapshot [apps/host-client/src/screens/DungeonScreen.tsx:979] — deferred, pre-existing. The meta cache is filled only in `renderFrame` (rAF) from `state.projectiles`; a projectile that spawns and hits between two delivered snapshots is never cached, so its impact VFX silently no-ops (no fallback). Rooted in snapshot cadence — Story 7.10 (`projectile:moved` streaming) is the real fix.
- [x] [Review][Defer] Buffed-player reconnect re-triggers the buff-onset pulse [apps/host-client/src/screens/DungeonScreen.tsx:1082] — deferred, minor cosmetic. A player who leaves while carrying `damageBuff` drops from `buffedPlayersRef`; on reconnect (statusEffects reconciled) they read as newly-buffed and re-pulse once. Harmless double-pulse.

**Merge-gate / process notes (not code defects):**
- Task 8.3 (Client-UX manual pass) is still **NOT RUN** — the Client-UX hook requires a live host + phone + sim session. Blocks the merge gate; must be done by a human before merge.
- `apps/host-client/src/vfx/index.ts` is a Dev-Notes **blocked path** (7.1's shipped API). The change is additive re-exports only and is disclosed in the File List, but per the ownership hook it needs a cross-context ack. Low risk.

**Dismissed (5):** (1) `projectile:hit` effect omits `gameState` from deps — the suggested fix would re-run the delta switch every snapshot, a *worse* bug; current deps are correct. (2) Buff-onset re-fires if `damageBuff` flickers out of a snapshot — speculative; snapshots are full reconciled state. (3) Dark Pact recast-while-buffed shows no onset — **unreachable**: cooldown 5000 ms > buff 4000 ms (balance.ts:55,204), so the buff always lapses ≥1 s before recast. (4) Per-player ref maps not pruned on leave — benign; ids never reused and `engine.remove(unknown)` is a verified safe no-op (engine.ts:47). (5) AC3 double-window-gating stricter than literal task wording — matches documented intent, not a defect.
...
| 2026-07-23 | Story 7.9 re-point: ability spatial geometry moved into a shared **ability presentation contract** in `shared-types` (ADR-0003). Dev Notes guidance updated — **import** `VOID_PULSE_ZONE_RADIUS_PX`/`ABILITY_DELIVERY`/`ABILITY_HIT_*` from `shared-types` instead of transcribing (no local `150`); refined rule is "never import `game-rules` *logic*". No AC/Task-status change. |
| 2026-07-23 | Implemented Souldrinker VFX (Tasks 1–7, 8.1, 8.2). New `souldrinker-vfx.ts` (planners + `classifyHpChanges` + `spawnSouldrinkerVfx`) and `ability-vfx-config.ts` (7.8 seam, PixiJS-free); `souldrinker-vfx.test.ts` (9 tests, all pass). `DungeonScreen.tsx` gains the Souldrinker suppress-and-replace `ability:fired` branch, a `projectile:hit` branch, `projectileMeta` on `VfxContext`, and snapshot-driven buff-onset + cost/gain classifier effects. `host-session.ts` whitelists `projectile:hit` (AC5). Geometry imported live from `shared-types`. `npm run typecheck` exit 0; host-client suite otherwise green apart from a **pre-existing** 7.2 Stonehide test failure (Story 7.9 range drift, verified independent of this story). Task 8.3 manual Client-UX pass **not run** (no live host+phone+sim session available) — left unchecked for a human. Status → review. Confidence 80%. |
