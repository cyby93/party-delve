...
1. **AC1 — Four distinct ability visuals.**
   ...
   **and** none of the four falls back to the shared flat `ABILITY_FLASH_MS` alpha-flash as its only treatment (the flash itself stays — see AC6 — but it is no longer the sole signal),
...
- [ ] **Task 2 — New host-side VFX modules (AC: 1, 2, 3, 4).**
  ...
  - [ ] 2.2: `apps/host-client/src/vfx/souldrinker-vfx.ts` — the **pure planner** (Task 3) plus a thin `spawnSouldrinkerVfx(engine, specs)` executor that translates each spec into the matching `create*` factory call and `engine.add(...)`. Keep the planner free of any `pixi.js` import so it unit-tests instantly.
...
- [ ] **Task 4 — Blood Spike (AC: 1, 2).**
  - [ ] 4.1: In the transient-delta `useEffect` (`:537-623`), extend the `ability:fired` branch: keep `entry.flashUntil = Date.now() + ABILITY_FLASH_MS` unchanged (AC6), then look up the caster via `gameState?.players.find(p => p.id === delta.playerId)`. If not found (late join / reconnect race) return without throwing. If `player.class !== PlayerClass.SOULDRINKER` fall through to the existing behavior only.
  - [ ] 4.2: `abilityIndex === 0` → `spawnSouldrinkerVfx(engine, planSouldrinkerCast({ abilityIndex: 0, ... }))`: the forward launch beam **plus** the self-cost droplet burst. Trigger the self-cost cue from `ability:fired` directly — Blood Spike *always* pays 10 HP on cast (`balance.ts:64`), so no HP-delta inference is needed here, which also makes it immune to the batching limitation below.
...
1. **CLOCK CONTRACT.** No primitive reads a clock. `startedAt` is captured from the **first `update(now)`**. Whatever you pass to `VfxEngine.update(now)` becomes every effect's clock. `renderFrame` uses `Date.now()` (`:99`); the purification-pulse and reward-particle blocks in the same ticker use `performance.now()` (`:465,486`). **Pass `Date.now()`** and never mix. A mixed clock makes effects vanish on frame 1 or leak forever, silently, with no exception. [`vfx/types.ts:26-34`]
