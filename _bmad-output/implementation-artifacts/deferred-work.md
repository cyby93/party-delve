...
All three named gaps closed:
1. **Cone/wedge shape** — new `createConeWedge` primitive (`primitives.ts`), wired into Stone Wall/Avalanche (`ability-vfx.ts`, apex-anchored wedge alongside the pre-existing displacement rings), Ancestor's Voice (`spiritcaller-vfx.ts`, replacing the old full-circle "expand ring" that contradicted the real 70° cone), and Crimson Lash (`souldrinker-vfx.ts`, the 3-beam fan re-derived from the live `coneAngleDeg` instead of a hardcoded `±0.30` rad literal).
2. **Chain-arc rendering** — `ability:chain-hit` is now whitelisted (`host-session.ts`) and consumed in `DungeonScreen.tsx`, drawing one beam per chain hop via a new `planChainHitBeam` (`stormcaller-vfx.ts`).
3. **Resized/re-timed projectile + blast burst** — the stale pre-3.26 fake 170ms hitscan-flight flourish (`HurlFlight`/`advanceHurlFlights`/`StormcallerCastPlan.flight`) is removed; a new `PlayerClass.STORMCALLER` entry in `PROJECTILE_APPEARANCE` (`ability-vfx-config.ts`) gives the real streamed projectile a body sized off `TEMPEST_HURL_PROJECTILE_RADIUS_PX`; a new `planTempestHurlImpact` (`stormcaller-vfx.ts`) drives the real `projectile:hit`-triggered impact sized to `TEMPEST_HURL_BLAST_RADIUS_PX`.

**Explicitly still deferred / not touched by this story** (do not read the above as resolving these too): `D-7.5-A` (`isStormEyeZone` matching any Stormcaller `'damage'` zone) — untouched, separately tracked above; Lightning Arc's own 30° corridor-search visualization — a deliberate non-goal (the corridor angle constant is host-forbidden per `project-context.md`'s Monorepo Ownership rule, only the resolved chain *hits* are drawn); Thunder Clap and Storm Eye — untouched, already correct, never named in ADR-0005's gap; the engine-wide VFX-pooling deferral (7.1's original D-7.1-D lineage) — this story's new effects are fire-and-forget and cast-cadence-bound like every prior Epic 7 story, so no new pooling need was introduced.

---

## Deferred from: code review of 7-13-cone-wedge-chain-lightning-and-tempest-hurl-vfx (2026-07-30)

**D-7.13-A — `projectile:expired` is not forwarded to the host for any projectile ability, so a miss/out-of-range projectile has no dedicated visual beyond the generic trail fade-out** [`apps/host-client/src/session/host-session.ts`'s `onTransientDelta` whitelist, `apps/simulation-server/src/rooms/GameRoom.ts:1977`, `packages/net-protocol/src/apply-delta.ts:266-268`]
`ProjectileExpiredDelta` already exists on the wire (broadcast by `GameRoom.ts` when `isProjectileExpired` fires, i.e. a projectile flies past `PROJECTILE_MAX_RANGE_PX` without hitting anything) and `apply-delta.ts` already has a real case for it (removes the projectile from `state.projectiles`, same as `projectile:hit`) — but it is not in `host-session.ts`'s whitelist, so `DungeonScreen.tsx` never gets a chance to render a dedicated miss/fizzle effect for it. The only visual today is the generic Projectiles-block cleanup-on-missing: the body `Graphics` is destroyed and the trail simply stops being fed, fading point-by-point over its own `durationMs` (the intentional "despawn read" documented at `DungeonScreen.tsx`'s Projectiles block since Story 7.8). Found by the Edge Case Hunter layer during Story 7.13's code review, specifically flagged against Tempest Hurl's new projectile — but verified during triage to be identical, pre-existing behavior for Blood Spike and Void Pulse too (this is not a Tempest Hurl-specific regression). Not fixed here: ADR-0005's named gap for this story was exactly "a cone/wedge shape, chain-arc rendering, and a resized/re-timed projectile + blast burst" — a miss/fizzle visual for any projectile ability was never part of that gap, and fixing it well means a host-only change affecting all three projectile abilities uniformly (a `projectile:expired` whitelist entry + a shared "miss puff" planner), which is a coherent unit of its own work. Revisit as its own small Epic 7 VFX story, or fold into a future `epic-7-post-*` hardening sweep.

---

## Deferred from: manual Client-UX pass of 7-13-cone-wedge-chain-lightning-and-tempest-hurl-vfx (2026-07-30)

**D-7.13-B — Lightning Arc's chain essentially never fires in real gameplay: typical enemy spacing far exceeds `LIGHTNING_ARC_CHAIN_RADIUS_PX` (150px)** [`packages/game-rules/src/balance.ts:142`, `apps/simulation-server/src/rooms/GameRoom.ts:1452-1483` (`handleLightningArc`)]
Raised by the user's own manual playtest of this story ("when the lightning hits an enemy it does not chain towards the next enemy — it should" / "I can't see damage numbers except on the first hit enemy"). Investigated live: wrote a temporary e2e harness (start a real simulation-server, join, run a grassland dungeon, fire Lightning Arc at the nearest enemy, log every `ability:chain-hit`/`enemy:damaged`) across two independent runs (easy and hard difficulty). Both runs spawned exactly 2 alive enemies at level start, at 887px and 302.6px apart respectively — both **far** outside the 150px `LIGHTNING_ARC_CHAIN_RADIUS_PX` the chain search (`resolveLightningArcChain`, called from `handleLightningArc`) uses to find a second target from the first hit's position. Both runs produced exactly one `ability:chain-hit` (`chainIndex: 0`) and one `enemy:damaged` — the sim is behaving exactly as coded (confirmed correct in isolation by the pre-existing `tests/unit/lightning-arc.test.ts`), it simply never had a qualifying second target to chain to. This is the most likely explanation for both symptoms the user reported: no second beam to render (nothing for the host's new `ability:chain-hit` VFX, Story 7.13 Task 5, to draw — verified separately working via unit tests: N same-tick calls each produce their own beam, no batching regression) and no damage number on a second enemy (no `enemy:damaged` delta was ever broadcast for one, since no hit landed there). **Not fixed here** — this is a `packages/game-rules`/balance tuning question (raise `LIGHTNING_ARC_CHAIN_RADIUS_PX`, or cluster enemy spawns tighter), a Blocked path for this VFX story, and orthogonal to ADR-0005's named VFX gap (which assumed chaining already happens mechanically and asked only for its *visual*). Revisit as a Simulation Engineer story if this is judged an actual gameplay problem — the two sampled layouts are not necessarily representative of every level/wave, so a broader sample (multiple waves, multiple difficulties) would be worth checking before retuning anything.

---

## Deferred from: code review of 3-27-ability-geometry-consolidation-per-ability-objects (2026-07-29)

**D-3.27-A — `gatherPlayersInHitZone`/`isInAbilityHitZone` use two different signals for "is this a cone"** [`apps/simulation-server/src/rooms/GameRoom.ts`]
`gatherPlayersInHitZone` discriminates cone-vs-circle via `geometry.coneAngleDeg !== undefined`; `isInAbilityHitZone` discriminates via `geometry.hitShape === 'cone'`. Nothing in the type system keeps these two signals in sync — this story's own Blocked paths explicitly rejected a discriminated union tying `coneAngleDeg` to `hitShape`. Pre-existing shape of these two helpers (predates Story 3.27 — the same dual-discriminant pattern existed across the equivalent pre-refactor call sites), not introduced by this diff. The new `tests/contract/ability-geometry-shape.test.ts` contract test now asserts `coneAngleDeg` presence and `hitShape === 'cone'` always agree for every real ability, which is the practical guard against drift. Revisit only if a future geometry entry is added by hand without going through that contract test.

**D-3.27-B — Three call sites hand-roll an identical inline "default hitscan circle" `AbilityGeometry` literal instead of sharing one constant** [`apps/simulation-server/src/rooms/GameRoom.ts`: Warding Cry's ally-gather, the Spirit Nova sweep, and the generic dispatch loop's out-of-range fallback]
`{ hitRangePx: 0, hitRadiusPx: ..., hitShape: 'circle', delivery: 'hitscan' }` is constructed inline at three separate call sites (two with a dynamic `hitRadiusPx`, one fully static). Minor duplication; extracting a shared factory was judged not worth the added abstraction for a rarely-touched code path, per this story's own minimal-churn mandate. Revisit only if a fourth call site needs the same shape, or if the "safe default" values ever need to change (currently would require finding and updating three places).

**D-3.27-C — `AbilityGeometry.coneAngleDeg?: number` can't distinguish "unset" from a legitimate future 0°-cone ability** [`packages/shared-types/src/ability-geometry.ts`]
The field is a plain optional (present only when `hitShape === 'cone'`), not a discriminated union — a deliberate, documented choice (this story's Dev Notes/ADR-0006 explicitly consider and reject the discriminated-union alternative as disproportionate for a mechanical migration). Harmless today since no ability has a real 0° cone angle. Revisit only if such an ability is ever designed.
...
**Geometry half RESOLVED by Story 3.27 (2026-07-29, ADR-0006):** `packages/shared-types/src/ability-geometry.ts`'s 5 flat tables (`ABILITY_HIT_RANGE_PX`, `ABILITY_HIT_RADIUS_PX`, `ABILITY_HIT_SHAPE`, `ABILITY_CONE_ANGLE_DEG`, `ABILITY_DELIVERY`) are replaced by one `AbilityGeometry` object per ability in a new `ABILITY_GEOMETRY` table — hard cutover, every read site (`GameRoom.ts`, 4 host-client VFX files, 10 test files) migrated atomically in the same story. The `AbilityBalance` half (`packages/game-rules/src/balance.ts`'s ~15 tables) remains open — split out during 3.27's creation because its only consumer is `GameRoom.ts` (single-owner Simulation Engineer, no cross-boundary approval needed), unlike the geometry half which spanned 3 ownership areas. Tracked as a new, separate, not-yet-created future story (3-28-ability-balance-consolidation).

**AbilityBalance half RESOLVED by Story 3.28 (2026-07-29):** `packages/game-rules/src/balance.ts`'s remaining 9 flat tables (`ABILITY_COOLDOWNS_MS`, `ABILITY_DAMAGE`, `ABILITY_HEAL_AMOUNT`, `ABILITY_SELF_COST_HP`, `ABILITY_HP_SCALED_DAMAGE`, `ABILITY_LIFESTEAL_PCT`, `ABILITY_CHAINED_ZONE`, `ABILITY_STATUS_EFFECT`, `ABILITY_DISPLACEMENT_STRENGTH`) are replaced by one `AbilityBalance` object per ability in a new `ABILITY_BALANCE` table — hard cutover, byte-identical values, every read site (`dispatchAbility`'s 4 lookups, `GameRoom.ts`'s ~12 read expressions, both real test-file consumers) migrated atomically in the same story, plus a new `tests/contract/ability-balance-shape.test.ts` byte-identical migration guard mirroring 3.27's `ability-geometry-shape.test.ts` pattern. Single-owner Simulation Engineer, no new ADR (`packages/game-rules/**` doesn't trigger CLAUDE.md's Contract-change hook). **D-CC1 is now fully closed** — both halves resolved.
...
When N deltas share one WS flush (e.g. `ability:fired` immediately followed by `player:downed` for the same player), every branch that reads `gameState` (ability VFX placement via caster position, etc.) evaluates against the single post-batch `gameState` prop value common to the whole effect call, not the state as of that specific delta's arrival — e.g. an ability's cast VFX could be placed using the caster's already-downed position rather than their position at cast time. This is a structural consequence of moving from one-delta-per-render to N-deltas-per-render while `gameState` stays a single closure value per render; it is explicitly out of this story's scope (Non-goals: "no change to any individual delta-handling branch's *logic*") since fixing it would require reconstructing a per-delta state snapshot, not a mechanical wrap. Revisit only if a real gameplay/VFX case exhibiting this staleness is observed.

**D-7.11-B — Same-WS-flush spawn+hit for a projectile is a distinct, deeper gap than the ticker-cadence case Task 3 fixes** [`apps/host-client/src/screens/DungeonScreen.tsx:1302-1319`, `apps/host-client/src/session/host-session.ts:42-91`]
If a projectile's spawn delta and its `projectile:hit` delta both land in the same WS flush, `applyDelta` removes it from the mirrored state before React ever renders — `gameState.projectiles` never contains it in any render, so neither `renderFrame`'s ticker-driven population nor Task 3's new `[gameState]`-keyed effect can ever cache its meta, and the `projectile:hit` cue silently no-ops exactly as before this story. AC4 promises to close the ticker-cadence gap (a projectile visible in `gameState` but missed by the ~60fps ticker between renders) — this is a different, narrower case where the projectile is never visible in `gameState` at all, which no population-effect approach can fix. A real fix would need a fallback path when `meta` lookup misses (e.g. deriving minimal display info directly from the `projectile:hit` delta's own fields) or a protocol-level change — both outside this story's Allowed paths and its explicit Non-goal against changing which deltas `host-session.ts` forwards. Revisit as a follow-up story if this case is observed to matter in practice.

**D-7.11-C — Revive-deadline effect's `[transientDeltaQueue, gameState]` dependency re-processes the same, not-yet-cleared queue on unrelated `gameState` renders** [`apps/host-client/src/screens/DungeonScreen.tsx:1379-1396`]
Because `gameState` updates far more often than the queue is cleared, any `gameState`-only re-render occurring before App's clearing effect runs re-executes this effect against the same queue contents, redundantly re-setting revive deadlines it already set. The dual-dependency shape (`[latestTransientDelta, gameState]`) existed in identical form before this story (pre-existing, not introduced by this diff) — this story's mechanical-wrap mandate didn't call for redesigning it. Revisit only if redundant deadline resets are ever observed to cause a visible timing issue.

**D-7.11-D — A screen unmount triggered by one item in a batch (e.g. `run:failed`) prevents DungeonScreen's effects from ever seeing any other item in that same batch** [`apps/host-client/src/App.tsx:79-81`, `apps/host-client/src/screens/DungeonScreen.tsx`]
`run:failed` flips `gameState.session.phase` to `'post-run'` synchronously before React's next render, so on the commit that includes it, `App` no longer renders `<DungeonScreen>` at all — any other same-flush item (e.g. a final `enemy:damaged`/`boss:damaged` from the same tick) is present in `transientDeltaQueue` but never reaches DungeonScreen's two dispatch effects, since the component doesn't exist to run them. Practical impact is muted: the screen tearing down its own render targets in the same transition means nothing would have been visibly renderable regardless, and this is an inherent consequence of a consumer component unmounting mid-batch, not a defect in the queue mechanism itself (App's own top-level effect is unaffected, since App never unmounts). Revisit only if a case is found where this visibly drops a cue that should have rendered before the transition.

**D-7.11-E — DungeonScreen's two dispatch effects have no cleanup function, so React StrictMode's dev-only double-invoke can double-process a queue present at initial mount** [`apps/host-client/src/screens/DungeonScreen.tsx:1003-1300,1379-1396`]
Neither effect returns a cleanup function, so in development-only StrictMode double-invoke (setup→cleanup→setup), if `transientDeltaQueue` is already non-empty at DungeonScreen's first mount, its branches run twice — e.g. `essence:dropped`'s `new Graphics()`/`app.stage.addChild` would create and orphan a duplicate Graphics object, or `enemy:damaged` would show a duplicate floating damage number. This pattern (no cleanup, no per-branch idempotency guard besides `boss:defeated`'s own ref) existed identically before this story for the single-item case — not newly introduced, just now potentially covering more items per invoke. Dev-only impact (StrictMode double-invoke doesn't run in production); out of scope for a "mechanical wrap, zero logic change" story. Revisit only if this is observed to cause a visible dev-mode glitch worth fixing.

**D-7.11-F — `bond:assigned`'s single-slot `setBondOverlay` state collapses two same-batch bond announcements to showing only the last one** [`apps/host-client/src/screens/DungeonScreen.tsx:1007-1014`]
`setBondOverlay` is a plain (non-functional) `useState` setter; if two `bond:assigned` deltas for two different pairs land in the same WS flush (plausible with 4 players forming both a proximity and a fate bond in one tick), the loop calls it twice and only the second call's value survives — the first bond's announcement text is never shown, even briefly. This is the same "same-batch collapse" defect class Story 7.11 fixes at the queue-delivery level, reappearing one layer up in downstream single-slot UI state that wasn't itself converted to a queue/list. The code already flags a sibling instance of this exact class as a known, deferred gap (the `:1008` comment re: `level:complete` vs. `bond:assigned`), just not this specific bond-vs-bond case. AC5 explicitly scopes this story's no-regression guarantee to "the single-delta-per-batch case," so composing multiple same-type deltas losslessly through every downstream single-slot UI state is a larger scope than what 7.11 promises. Good candidate for a small follow-up (e.g. convert `bondOverlay` to a short queue/list of pending announcements) if simultaneous same-tick bonds are ever observed in practice.
...
**Answered by Story 7.6** (`7-6-status-effect-vfx`, 2026-07-25): status auras are the first consumer this deferral explicitly named ("revisit when 7.2-7.8 reveal real effect volumes"). Design bound: at most one live `EffectHandle` per `(entityId, effectType)` pair, plus at most one handoff frame of overlap for the three cadence kinds (`damageReduction`/`shield`/`damageBuff` re-trigger without an explicit `remove()` on the outgoing pulse). Worst realistic case with today's abilities: `damageReduction` self-only (Iron Skin) ≤ 8, `damageBuff` self-only (Dark Pact) ≤ 8, `shield` allies-only (Warding Cry) ≤ 8, `slow` enemies-only (Tremor Stomp) ≤ ~20 alive enemies in a survive-waves level ⇒ **≈ 44 concurrent handles**, ~20 of them zero-allocation persistent trails. Allocation rate: ≤ 24 cadence handles re-triggering at ≥ 700 ms ⇒ ≤ ~34 handle creations/second worst case (comparable to the existing reward-reveal burst's 8-12 `Graphics` in a single frame). A defensive `MAX_STATUS_AURAS = 64` ceiling load-sheds new aura creation beyond it (warns once via a module-level flag, never per-frame) — existing auras keep running, only new ones are shed. **Verdict: pooling stays deferred** for status auras specifically; bounded per-pair allocation plus the hard ceiling is sufficient at these volumes, and a pool would add lifecycle complexity to `VfxEngine` (interacting badly with D-7.1-A's double-add class) for no measurable gain at ~44 handles. The engine-wide deferral (other Epic 7 consumers: 7.2-7.5, 7.7b, 7.8) remains open — 7.6 answers only the status-aura slice of it.
...
## Deferred from: dev implementation of 7-7a-boss-charged-delta-contract-and-broadcast (2026-07-26)

**D-7.7a-A — Boss charge does not re-sync the planck physics body** [`apps/simulation-server/src/rooms/GameRoom.ts` — `case 'boss:moved'` calls `this.bossBody.setPosition(...)`, `case 'boss:charged'` does not]
`case 'boss:moved'` repositions `this.bossBody` via `setPosition(Vec2(toMeters(evt.x), toMeters(evt.y)))` after every chase-tick move, but `tryCharge`'s single-tick lunge (`grassland-boss.ts:78-91`) only mutates `this.gameState.boss.position` — the charge path never touches `this.bossBody`. Result: after a charge, `this.gameState.boss.position` and the physics body disagree by up to `min(len, BOSS_CHARGE_SPEED × dt) ≈ 11.7 px` until the next `boss:moved` tick re-syncs them (at most one 33 ms tick later, per the boss's chase-cadence). Identified and explicitly scoped out during Story 7.7a (`boss:charged` delta contract + broadcast) — repositioning the physics body would change collision resolution, which is a boss-behaviour change outside 7.7a's Non-goals ("no change to boss behaviour, cooldowns, phases, damage, movement or physics") and belongs to the Simulation Engineer with its own full Simulation-safety review. Revisit only if a real gameplay symptom is observed (e.g. a player standing exactly at the charge's post-lunge position experiencing a stale collision for one tick); at ≤ 11.7 px against player/boss hitbox sizes this is expected to be imperceptible in practice.

## Deferred from: code review of 7-7a-boss-charged-delta-contract-and-broadcast (2026-07-26)

**D-7.7a-B — `GameRoom.ts`'s boss-event switch has no exhaustive/`never` guard** [`apps/simulation-server/src/rooms/GameRoom.ts:1919`, the `switch (evt.type)` over the local `BossEvent` union from `grassland-boss.ts`]
Unlike `packages/net-protocol/src/apply-delta.ts`'s switch over `DeltaEventMsg` (which has a `default: const _exhaustive: never = evt` guard), the boss-event switch inside the tick loop has no such guard. A future `BossEvent` variant added to `grassland-boss.ts`'s `BossEvent` union would compile cleanly with no corresponding `case` here, and the emitted event would be silently dropped by the `for` loop — no broadcast, no compile error, no test failure. This is not hypothetical: `boss:charged` itself sat in exactly this state before Story 7.7a (case present, only a `break;`, no broadcast) for an unknown number of prior stories — proof the switch's completeness has depended entirely on developer diligence, not the type system, all along. Found by the Edge Case Hunter layer during Story 7.7a's code review; pre-existing, not introduced by that story's diff (which only touched the body of the existing `case 'boss:charged'`). Fixing it means adding a `default`/`never`-exhaustiveness arm to a switch this story was explicitly scoped not to touch beyond its one case. Revisit alongside any future boss-event addition, or as a dedicated small hardening story.

## Deferred from: code review of story-7-7b-grassland-boss-attack-vfx-and-charge-visual (2026-07-26)

**D-7.7b-A — `applyBossVfxPlan`'s `tintId` return only tracks the last `tint` descriptor in a plan** [`apps/host-client/src/screens/DungeonScreen.tsx`, `applyBossVfxPlan`, case `'tint'`]
The bridge function returns a single `number | null` for the `tint` descriptor's effect id, overwriting `tintId` on every `tint` case it encounters in the loop. Not reachable today — only `boss:phaseChanged` ever emits exactly one tint per plan — but if a future boss reaction emitted two tint descriptors in one plan, the first would be silently orphaned: never cancelled, leaking (visually) until its own `durationMs` naturally expires. Found by Blind Hunter + Edge Case Hunter (both independently). Fixing it now would mean changing the return type to an array (or a list of ids) and updating every one of the four call sites' single-ref (`bossTintEffectIdRef`) cancellation pattern to match — disproportionate to a currently-unreachable case. Revisit only if a future boss reaction is designed to emit more than one simultaneous tint pulse.

**D-7.7b-B — Zero automated coverage of `applyBossVfxPlan` and the `DungeonScreen.tsx` boss wiring branches** [`apps/host-client/src/screens/DungeonScreen.tsx`]
All 5 of this story's new tests (`boss-vfx.test.ts`) exercise the pure `planBossVfx` function only. The bridge function (`applyBossVfxPlan`: switch-on-`kind` → factory call → `engine.add` → tint-id return) and the four `else if` branches wiring `boss:charged`/`boss:stomped`/`boss:phaseChanged`/`boss:damaged` into `DungeonScreen.tsx` — including the tint-cancellation-at-two-destroy-sites logic and the damage throttle — have no automated test. Found by Blind Hunter. This matches the project's established testing convention (this story's own Testing Standards section: "one runnable check for load-bearing logic, not a suite per function," rendering verified via the manual Client-UX pass instead), so it is not a gap unique to this story — but the manual Client-UX pass (Task 8.4) that convention relies on to cover this exact code was **not performed** in this implementation session (no display available in this sandbox, consistent with prior-story precedent: 7.5, 7.6, 3.23, dev-3). Already disclosed in the story's own Completion Notes; reflected in its `review` (not `done`) status. Revisit: perform the Task 8.4 manual Client-UX pass before closing this story to `done`.

## Deferred from: code review of 7-12-epic-7-post-710-deferred-hardening-simulation (2026-07-27)

**D-7.12-A — `boss:charged` broadcasts a boss position with no finite guard, unlike its now-guarded `boss:moved` sibling** [`apps/simulation-server/src/rooms/GameRoom.ts:1960-1967`, `case 'boss:charged'`]
Story 7.12 added a `Number.isFinite` guard to `case 'boss:moved'` (`GameRoom.ts:1935-1938`) per AC2, but AC2's scope was literally limited to `enemy:moved`/`boss:moved` — it never named `case 'boss:charged'`. `tryCharge` (`packages/game-rules/src/entities/grassland-boss.ts:78-91`) mutates `boss.position.x/y` via the exact same `dx/dy/len`-division arithmetic pattern as `tickBossChase` (`grassland-boss.ts:109-128`, the source of `boss:moved`) — if that arithmetic path ever produced a non-finite value, `boss:moved` would now be suppressed but `boss:charged` would still broadcast the non-finite coordinate unguarded in the very same tick, one case over. Found by the Edge Case Hunter layer during Story 7.12's code review. Fixing it means extending the identical guard pattern to `case 'boss:charged'` (and auditing `case 'boss:stomped'` — `tryStomp`, `grassland-boss.ts:95-100` — for the same exposure, since it also reads `boss.position.x/y` directly) — deliberately deferred rather than silently patched in-review since it's outside this story's literal AC2 enumeration and CLAUDE.md's "do not silently expand scope" guidance. Revisit as a small follow-up hardening story, or fold into the next `epic-7-post-*` sweep.

**D-7.12-B — `enemy:stomped` events bypass the finite guard entirely** [`apps/simulation-server/src/rooms/GameRoom.ts:1900-1914`]
The finite check Story 7.12 added is nested inside `if (aiEvt.type === 'enemy:moved')` (`GameRoom.ts:1903`), so any other `EnemyAIEvent` variant in the same `result.value` array — notably `enemy:stomped` from `StompLayer.execute()` (`packages/game-rules/src/systems/ai/layers/stomp.ts:14-21`, which carries `x: enemy.x, y: enemy.y`) — skips the check completely and reaches the unconditional `this.broadcast(EventNames.DELTA, delta)` at `:1914` regardless of whether `enemy.x`/`enemy.y` are finite. Found by the Edge Case Hunter layer during Story 7.12's code review. Same rationale for deferring rather than patching now as D-7.12-A: outside AC2's literal `enemy:moved`/`boss:moved` enumeration, but relevant to the story's own stated intent ("a non-finite coordinate can never leak to clients"). Revisit alongside D-7.12-A.

## Deferred from: code review of 2-11-class-pick-button-fit (2026-08-03)

**D-2.11-A — No maxHeight/overflow guard on the confirm button for label text longer than the current fixed string** [`apps/mobile-controller/src/screens/ControllerScreen.tsx:469-497`]
The button (`minHeight: 48`, no `maxHeight`) and its wrapper column (`flex: '0 0 20%'`, stretched to the panel's fixed `height: 130`) have no explicit cap or overflow/ellipsis fallback for a wrapped label. Not reachable with the current fixed "Pick Selected Class" string — worst-case wrap (~43px at 3 lines) fits comfortably inside the ~110px content budget (130px panel minus 10px+10px padding), confirmed by layout math during the Acceptance Auditor pass. Found by the Edge Case Hunter and Blind Hunter layers (independently). Copy changes are an explicit non-goal of Story 2.11, so this wasn't patched in-review. Revisit if the label is ever localized or lengthened.

**D-2.11-B — Degenerate one-character-per-line wrapping theoretically possible on extremely narrow viewports** [`apps/mobile-controller/src/screens/ControllerScreen.tsx:457-467`]
`word-break: break-word` can force near-single-character wrapping if the wrapper column's content width (20% of viewport, minus padding/border) shrinks below what a single word needs. Found by the Edge Case Hunter layer. Outside this project's realistic target device range (phone controllers, not smartwatches) — no minimum-viewport spec exists to design against, so this wasn't patched. Revisit only if a real device report surfaces this.

**D-2.11-C — `fontSize: 11` mobile legibility at a glance** [`apps/mobile-controller/src/screens/ControllerScreen.tsx:486-492`]
Found by the Blind Hunter layer as a legibility concern for a small, now-potentially-multi-line button label on a phone controller. Pre-existing, untouched by this diff (Story 2.11 is scoped to a wrap-only CSS fix, not a font-size change) — a general Client-UX concern for the whole confirm button, not introduced by this story. Revisit as part of a broader mobile-controller typography pass if flagged during manual Client-UX testing.

**D-2.11-D — Inline duplicated typography styling instead of a shared ui-kit text/button component** [`apps/mobile-controller/src/screens/ControllerScreen.tsx:484-496`]
Found by the Blind Hunter layer. The confirm button's label style hand-rolls `fontFamily`/`fontWeight`/`fontSize`/`letterSpacing`/`lineHeight` inline rather than via a shared `packages/ui-kit` component, so the same wrap fix would need re-applying to any other button with an equivalent overflow problem. Pre-existing pattern used throughout the file; out of scope for a single-property CSS fix. Revisit if a shared button/text component is introduced for `apps/mobile-controller/**`.

**D-2.11-E — No automated (unit/visual-regression) test coverage for this UI-fit fix** [`apps/mobile-controller/**`]
Found by the Blind Hunter layer. Matches the project's already-accepted, pre-existing gap — `apps/mobile-controller` has zero component-test precedent across every prior mobile-controller story, explicitly called out as expected (not a gap introduced by this story) in Story 2.11's own Dev Notes and Required Tests section ("None new"). Revisit only if/when the project adopts component-testing tooling for `apps/mobile-controller/**`.

## Deferred from: code review of 4-15b-abandon-run-resolution (2026-08-04)

**D-4.15b-A — Missing/falsy `accept` field in `run:abandon-vote` is silently treated as an explicit decline** [`apps/simulation-server/src/rooms/GameRoom.ts:74-77`]
`if (!msg.accept) { cancelAbandonProposal(...); return; }` can't distinguish "player declined" from "malformed/partial payload" (missing field, `undefined`, `0`). Found by the Blind Hunter layer. Not a regression: mirrors the existing `run:vote` handler's identical `if (!msg.accept)` design (`GameRoom.ts:244`), which this story's Dev Notes explicitly directed reusing verbatim. Revisit only if the codebase ever adopts stricter payload validation across all `onMessage` handlers — this would not be a story-4.15b-specific fix.

**D-4.15b-B — `bond:continue` (CONTINUE handler) doesn't check for a pending `abandonProposal` before loading the next level** [`apps/simulation-server/src/rooms/GameRoom.ts:361-374`]
Per AC1 there is deliberately no bond-moment gate on proposing an abandon, so a proposal can be pending when `bond:continue` fires; the handler still loads the next level regardless. Found by the Edge Case Hunter layer. Benign: `resetToHub()` unconditionally tears down whatever level state exists regardless of when it was loaded, so an immediately-following unanimous accept still resolves cleanly to hub — no state corruption, no stuck proposal. Revisit only if playtesting surfaces a confusing "level changed out from under my pending vote" UX moment once Story 4.15c's mobile UI ships.

**D-4.15b-C — Guard branches added by this story have no direct test coverage beyond AC6's four named scenarios** [`tests/e2e/abandon-run.test.ts`]
Untested: propose/vote from a frozen player, propose outside `'dungeon'` phase, the voluntary consented-leave cancellation path, and a malformed/missing-field vote payload. Found by the Blind Hunter layer. AC6 named exactly four scenarios (unanimous-accept, single-decline, disconnect-during-vote, no-boss-gate) and all four are covered; broader guard coverage was not requested by any AC and would be scope creep on a story whose Tasks/Subtasks were followed verbatim. Revisit as a small follow-up hardening pass if any of these untested paths is ever suspected of a regression.

**D-4.15b-D — All abandon-handler guard failures are silent bare `return`s with no NACK/feedback to the triggering client** [`apps/simulation-server/src/rooms/GameRoom.ts:262-292`]
Every existing `onMessage` handler in `GameRoom.ts` (RUN_PROPOSE, VOTE, CLASS_SELECT, etc.) follows the same silent-discard-on-invalid-guard pattern; no NACK protocol exists anywhere in the file today. Found by the Blind Hunter layer. Pre-existing codebase-wide convention, not introduced by this story. Revisit only alongside a dedicated protocol-level NACK/error-feedback design (a `net-protocol` change — Protocol Architect ownership, contract-change hook), not as a one-off fix inside this story's two handlers.

## Deferred from: code review of 4-15c-leave-run-button-and-vote-ui (2026-08-05)
...
**D-LAN-A — QR join URL still depends on the server self-reporting its IP via `/local-ip`** [`apps/host-client/src/screens/LobbyScreen.tsx:17`] — **CLOSED 2026-08-12**
`getLocalIp()` (`apps/simulation-server/src/index.ts:18-27`) picks the first non-internal IPv4 interface. On WSL2 the only such interface is the NAT address (e.g. `172.25.98.210`), which is unreachable from phones on the real LAN, so the QR encodes a dead URL even though session creation now succeeds. `window.location.hostname` is reachable by construction whenever the operator opened the host page over the LAN, but falls back to `localhost` when the operator uses `localhost:5173` on the sim machine — which is exactly why `/local-ip` exists. Fixed as described: `pageHost()` returns the page hostname unless it matches `LOOPBACK`, and only then does the `/local-ip` probe run. Note this does **not** make WSL2 NAT mode usable — Windows forwards `localhost` to WSL but not the LAN IP, so the host page there is reachable only at `localhost:5173`, which is precisely the loopback case that still falls through to the probe. Running the dev servers outside WSL, or switching WSL to `networkingMode=mirrored`, remains the prerequisite for phone joins.

**D-LAN-B — `VITE_MOBILE_URL` override is ignored whenever `/local-ip` succeeds** [`apps/host-client/src/screens/LobbyScreen.tsx:44`] — **CLOSED 2026-08-12**
The ternary only consulted `VITE_MOBILE_URL` when `mobileHost` was `null`, so an operator who explicitly pinned the mobile URL was silently overridden by whatever the server reported. Fixed alongside D-LAN-A: `MOBILE_URL_OVERRIDE` now outranks both auto-detection paths and suppresses the probe entirely.
