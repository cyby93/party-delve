...
- [x] **Task 1: One-time `VfxEngine` wiring in `DungeonScreen.tsx` — guarded "if not already present"** (AC: 1, 6)
  - [x] 1.1: Nothing consumes `apps/host-client/src/vfx/` yet (7.1 AC3 deliberately skipped integration). **Before writing anything, grep `DungeonScreen.tsx` for `VfxEngine`.** If a sibling story (7.3–7.8) already landed the wiring, reuse it verbatim and skip to Task 2. Do **not** create a second engine instance.
  - [x] 1.2: If absent: `import { VfxEngine, createRingShockwave, createBeam, createParticleBurst } from '../vfx';` alongside the existing pixi imports (`DungeonScreen.tsx:1-6`).
  - [x] 1.3: Add `const vfxEngineRef = useRef<VfxEngine | null>(null);` next to the other refs (`:369-392`).
  - [x] 1.4: In `initPixi`, after `pixiAppRef.current = app;` (`:417`) and before `app.ticker.add(...)`: `vfxEngineRef.current = new VfxEngine(app.stage);` — `app.stage` structurally satisfies `VfxStage` (`vfx/types.ts:21-24`).
  - [x] 1.5: In the ticker callback, **after** the `renderFrame(...)` call (`:421-433`) and before the boss-sprite block: `vfxEngineRef.current?.update(Date.now());` — exactly one call per frame. **`Date.now()`, not `performance.now()`** (see CLOCK CONTRACT pitfall in Dev Notes).
  - [x] 1.6: In the unmount cleanup (`:502-525`), **before** `app.destroy(...)`: `vfxEngineRef.current?.clear(); vfxEngineRef.current = null;`. Ordering matters — `clear()` calls `stage.removeChild` on a live stage.
  - [x] 1.7: Note the ticker's early return `if (!state) return;` (`:419-420`). The engine update sits after `renderFrame`, so on a null state no effect advances. Acceptable: no effect can have been triggered without a state either. Do not restructure the guard.

- [x] **Task 2: Add the pure ability→visual-config mapping module** (AC: 1, 2, 3, 5)
  - [x] 2.1: New file `apps/host-client/src/vfx/ability-vfx.ts` (kebab-case, inside the existing vfx module — keeps `DungeonScreen.tsx`'s delta handler thin and the mapping unit-testable without a canvas).
  - [x] 2.2: Export the Stonehide palette constants (`STONEHIDE_OCHRE`, `STONEHIDE_DUST`, `STONEHIDE_SLATE`) and the per-ability config table exactly as specified in **Per-Ability Visual Spec** below. No magic numbers in the delta handler.
  - [x] 2.3: Export `getAbilityVfxConfig(playerClass: PlayerClass, abilityIndex: number): AbilityVfxConfig | null` — returns `null` for every class/index that has no bespoke VFX yet. `null` is the single source of truth for "fall back to `ABILITY_FLASH_MS`", which is what keeps AC3's other-classes-unchanged promise mechanical rather than hand-maintained.
  - [x] 2.4: Export `resolveAbilityVfxPlacement(config, casterX, casterY, directionX, directionY): { casterX, casterY, hitX, hitY } | null` — normalizes the direction the same way the sim does (`GameRoom.ts:2108-2110` uses `normDirX/normDirY`), computes `hit = caster + normDir × config.hitRangePx`, and returns **`null`** when `magnitude === 0 && hitRangePx > 0` (the sim skips the hit in exactly that case — `GameRoom.ts:2203` — while still having broadcast the delta at `:2054-2062`, so a zero-direction Avalanche must render nothing rather than fire a zero-length beam at the caster's feet).
  - [x] 2.5: Guard `abilityIndex` against out-of-range values from a malformed delta — return `null`, never throw. The delta handler runs inside a `useEffect`; an exception there breaks every later branch.

- [x] **Task 3: Trigger the four one-shot cast effects from the `ability:fired` delta branch** (AC: 1, 2, 3, 5)
  - [x] 3.1: In the transient-delta `useEffect` (`DungeonScreen.tsx:537-623`), extend the existing `else if (latestTransientDelta.type === 'ability:fired')` branch (`:551-553`). Do not add a second branch for the same type — the chain is `if/else if`, a duplicate would be dead code.
  - [x] 3.2: Resolve the caster from state, not from the delta: `const caster = gameState?.players.find(p => p.id === latestTransientDelta.playerId);`. `AbilityFiredDelta` carries only `{ playerId, abilityIndex, directionX, directionY }` — **no class**. Handle `caster === undefined` (late join / reconnect race) by falling through to the existing `flashUntil` path.
  - [x] 3.3: `const cfg = caster ? getAbilityVfxConfig(caster.class, latestTransientDelta.abilityIndex) : null;` — if `cfg` is `null`, run the **existing** `entry.flashUntil = Date.now() + ABILITY_FLASH_MS` line unchanged. If `cfg` is non-null, **skip** `flashUntil` entirely (AC3) and trigger the configured handles.
  - [x] 3.4: `const place = resolveAbilityVfxPlacement(cfg, caster.x, caster.y, delta.directionX, delta.directionY);` — bail (render nothing, no flash) when it returns `null`.
  - [x] 3.5: Compose handles in the order `ring → beam → burst` and hand each to `vfxEngineRef.current?.add(...)`. Do **not** pass `startedAt` — let each handle capture its clock from the first `update(now)` (7.1's clock contract, `vfx/types.ts:26-34`).
  - [x] 3.6: Use `caster.x/caster.y` from the `gameState` snapshot, not a stale ref. The effect's dependency array is `[latestTransientDelta]` (`:623`) and `gameState` is captured from the closure — this is the same pattern the existing `enemy:damaged` branch already relies on (`:560-577`), so it is consistent, but note the caster may be ~1 frame stale. Acceptable for a 140–420 ms effect; do not add `gameState` to the dependency array (it would re-fire every delta on every state tick).

- [x] **Task 4: Persistent Iron Skin shell, driven from `GameState`** (AC: 4, 6)
  - [x] 4.1: Add `const ironSkinGraphicsRef = useRef<Map<string, Graphics>>(new Map());` beside the other graphics maps (`:371-378`).
  - [x] 4.2: Extend `renderFrame`'s signature with **one** new trailing parameter — an object, not a 10th positional `Map`: `vfxRefs: { ironSkinGraphics: Map<string, Graphics> }`. `renderFrame` has exactly one call site (`:421`), so this is a two-line change. The `VfxEngine` itself is **not** passed into `renderFrame` — one-shot effects are triggered from the delta handler and advanced from the ticker; `renderFrame` stays a pure state→screen redraw.
  - [x] 4.3: New `renderFrame` section, placed **after** the status-badge section (`:258-288`) so the shell layers below the badges in draw order: for each player, find `player.statusEffects.find(e => e.type === 'damageReduction')`. Render the shell only when the player's class is `PlayerClass.STONEHIDE` **and** that effect exists **and** `!player.isDown` **and** `!(player.isSpirit && !isPurified)` (a defensive buff on a corpse or a spirit is a lie; `isPurified` is already a `renderFrame` parameter).
  - [x] 4.4: Create-on-first-seen / cleanup-on-missing over `vfxRefs.ironSkinGraphics`, identical in shape to `statusBadgeGraphics` (`:266-288`): prune ids no longer qualifying (`app.stage.removeChild(g); g.destroy(); map.delete(id)`), then create + draw for the qualifying ones with `app.stage.addChild(g)`.
  - [x] 4.5: Per-frame draw (mutate in place — `g.clear()` then redraw on the *same* object, never `new Graphics()` per frame):
    ```ts
    ...
    ```
  - [x] 4.6: `IRON_SKIN_SHELL_RADIUS = 30`. Derivation, do not change casually: `PLAYER_RADIUS` is 24 (`:25`) and the status badge row sits at `y = -(PLAYER_RADIUS + 14) = -38` with `STATUS_BADGE_RADIUS = 6` (`:33, :283`), i.e. its lowest pixel is at `-32`. A shell at 30 leaves a 2 px gap under the badges and never occludes them (Client-UX readability check).
  - [x] 4.7: `expiresAtMs` is host-epoch ms (`packages/shared-types/src/status-effect.ts:6`) but is stamped by the server clock — clamp `fade` to `[0, 1]` as above so host/server skew can only mis-time the fade, never produce a negative alpha or a `NaN`. Do not remove the shell on `remaining <= 0`; removal is the sim's job (the effect leaves `statusEffects` and 4.4's prune handles it).
  - [x] 4.8: Clear the map in the unmount cleanup (`:519-524`) alongside the other `*.clear()` calls.

- [x] **Task 5: Composition contract with Story 7.6 (`damageReduction` aura)** (AC: 4)
  - [x] 5.1: This shell is deliberately **class-gated** (`player.class === PlayerClass.STONEHIDE`). Export the gate as a named predicate from `ability-vfx.ts` — `export const ownsIronSkinShell = (p: { class: PlayerClass; statusEffects: StatusEffect[] }) => boolean` — so 7.6 can negate it in one place instead of re-deriving the condition.
  - [x] 5.2: Add a comment at the shell's render block stating the contract verbatim: *7.2 owns Stonehide's Iron Skin cast-and-persist visual; 7.6 owns the generic four-type status treatment. If 7.6 ships second, its `damageReduction` aura must exclude players for which `ownsIronSkinShell()` is true, or replace this shell outright and delete this block — never render both.*
  - [x] 5.3: Do **not** touch the existing `STATUS_EFFECT_COLORS` badge rendering (`:38-44, :258-288`). It is 7.6's surface; the shell sits below it and coexists.
  - [x] 5.4: Do **not** render anything on *enemies* for Tremor Stomp's `slow` (0.4 / 2000 ms, `balance.ts:204-208`). Enemy-side status visuals are 7.6's scope; this story's stomp conveys the slow through the weight of its own effect only.
  - [x] 5.5: Story 7.8 owns projectile and zone rendering. Stonehide has **neither** — all four abilities are `'hitscan'` (`balance.ts:130`) and none chains a zone. There is no overlap to negotiate; do not touch the projectile (`:290-309`) or zone (`:311-330`) blocks.

- [x] **Task 6: Self-check** (AC: 1, 2, 3, 5)
  - [x] 6.1: New test file `apps/host-client/src/vfx/ability-vfx.test.ts` (leave 7.1's 24 tests in `vfx.test.ts` untouched). Run with `npx vitest run src/vfx/ability-vfx.test.ts` from `apps/host-client`. Node environment, no jsdom — `pixi.js` imports fine under plain vitest (7.1 proved this; the first collect is ~35 s on WSL2).
  - [x] 6.2: Cover, in one file: (a) all four Stonehide indices return a non-null config and no two configs are shape+color+motion identical; (b) every other class/index returns `null` (the AC3 fallback contract); (c) an out-of-range `abilityIndex` returns `null` instead of throwing; (d) `resolveAbilityVfxPlacement` puts Stone Wall / Tremor Stomp / Iron Skin at the caster and Avalanche at exactly `caster + normDir × 200` for a non-unit input direction (prove the normalization); (e) it returns `null` for a zero direction on Avalanche but a valid placement for a zero direction on Tremor Stomp; (f) every config's longest `durationMs` is strictly less than that ability's cooldown (the AC5 budget invariant — hardcode the four cooldowns as local test literals, **do not import `packages/game-rules`**).
  - [x] 6.3: `npm run typecheck` at repo root (covers all 10 tsconfigs).
  - [ ] 6.4: **NOT DONE** (corrected 2026-07-22 — was wrongly checked in the bulk task sweep; the Completion Notes always said PENDING). Client-UX hook manual pass — see the checklist in Dev Notes. Record the result in the Dev Agent Record; do not claim a check passed that was not actually run.
  - [x] 6.5: If the Avalanche effect volume or the batching collapse turns out to be visibly wrong in the manual pass, log it to `deferred-work.md` as a new `D-7.2-*` item rather than expanding this story into App-level plumbing.

### Review Findings (code review 2026-07-22 — Blind Hunter + Edge Case Hunter + Acceptance Auditor)

- [x] [Review][Decision] **RESOLVED — fixed both ways.** `startedAt: Date.now()` is now stamped at trigger for all three handle types, and `VfxEngine.update()` moved above the `if (!state) return;` guard. This deliberately overrides **Task 3.5** ("do not pass `startedAt`") and **Task 1.7** ("do not restructure the guard") — both overrides are review-sanctioned and commented at their call sites. `startedAt` still comes from `Date.now()`, the same clock `update()` uses, so 7.1's CLOCK CONTRACT is honoured, not bypassed. Original finding: **Effects added while the ticker is not advancing never start, never expire, and accumulate on the stage** — Per 7.1's CLOCK CONTRACT a primitive captures `startedAt` on its *first* `update(now)`. PixiJS drives `app.ticker` from `requestAnimationFrame`, which the browser stops for a hidden/minimised tab, while the WebSocket keeps delivering `ability:fired` and the React effect (not rAF-gated) keeps calling `engine.add(...)`. Handles are on the stage already (`engine.ts:24`), so nothing is reaped: ~3–10 `Graphics`/`Container` per second retained for the whole backgrounded period, then every one of them starts in the same frame on resume. Same root cause as the `if (!state) return;` guard sitting above `vfxEngineRef.current?.update(Date.now())` (`DungeonScreen.tsx:495-500`): effects live at the moment `gameState` goes null freeze instead of completing. The legacy `flashUntil` path and the sibling `damageNumberGraphics` path both use absolute `Date.now()` deadlines and self-expire on the first frame after resume; this is the only path that does not. Fixing it means overriding either Task 3.5 ("do not pass `startedAt`") or Task 1.7 ("do not restructure the guard"). [blind+edge]
- [x] [Review][Decision] **RESOLVED — dismissed, keeping the spec-literal behaviour.** Task 3.4's "render nothing" stands: a cast whose placement resolves to `null` draws no effect *and* no flash. The engine-null case was split out and patched separately, since that is an init/teardown race rather than a deliberate skip. Original finding: **A Stonehide cast that resolves to no placement now renders nothing at all — not even the legacy cast flash** — `resolveAbilityVfxPlacement` returns `null` for a zero-direction Avalanche, and the `else` branch then falls through with zero side effects (`DungeonScreen.tsx:649-651`). This is Task 3.4 working as written, but the trigger is the *common* case, not an exotic one: `SkillCell`'s AUTO handler seeds `lastDirX/lastDirY = 0` on touchstart and fires every 33 ms (`ControllerScreen.tsx:685-697`), so a tap-and-hold whose finger has not moved yet sends `(0,0)`. The server accepts it, burns the full 1000 ms cooldown, and broadcasts the delta — the player gets a dead second with no on-screen response, where the old flat flash at least confirmed the input. Decision: keep spec-literal silence, or restore the legacy flash as input feedback when placement resolves to `null`. [blind+edge]
- [x] [Review][Defer] **DEFERRED as `D-7.2-D`** — reason: spec defect in the AC2 table, not a code defect; the code faithfully implements what the spec specified. Original finding: **Iron Skin's 120 px cast ring depicts a hit zone the simulation never tests — the same trap AC2 caught one row earlier** — `GameRoom.ts:2194` is `if (rawDamage <= 0 && healAmount <= 0) continue;` and runs *before* the `isInHitZone` loop. Iron Skin has `ABILITY_DAMAGE.stonehide[2] === 0`, no heal, and a `self`-scoped effect, so hit-scan is skipped entirely and `ABILITY_HIT_RADIUS_PX.stonehide[2] = 120` is dead data exactly like Tremor Stomp's `hitRange: 160`. AC2's own table ("Iron Skin = radius 120 at the caster") is therefore wrong. Visual impact is mild — a converging slate ring reads as armour closing, not as reach — but the story's mechanical-honesty claim does not hold as written. Decision: retune the ring's `startRadius` to something not claiming a zone, or keep the visual and correct the AC2 table. [auditor]
- [x] [Review][Patch] **Task 6.4 is checked `[x]` but the Client-UX manual pass was never run** — the Completion Notes say so explicitly ("NOT YET RUN … all seven checklist lines PENDING"), so the checkbox and the "42/42 complete" claim are both false. 41 were done. [`7-2-stonehide-ability-vfx.md`:88]
- [x] [Review][Patch] **Shell layering: `addChildAt(g, 0)` puts the shell below *everything*, not below sprites** — index 0 is the absolute bottom, and bodies (`:168`), tethers (`:260`) and zones (`:381`) also insert at 0, so anything created in a later frame lands *beneath* the shell while anything already there paints *over* it. Task 4.4 specified plain `addChild`, and the shell's stroke (r 28–35) never overlaps the badge row's opaque geometry, so the deviation buys nothing. [`DungeonScreen.tsx`:332]
- [x] [Review][Patch] **A null `vfxEngineRef` silently swallows the cast** — `if (place && engine)` has no `else`, so a delta arriving before `await app.init()` completes, or after teardown nulls the ref, produces neither VFX nor the legacy flash. Unlike `place === null` this is an implementation state, not a deliberate "the sim skipped it" case, and should fall back to `flashUntil`. [`DungeonScreen.tsx`:651]
- [x] [Review][Patch] **The fade clamp advertised as NaN-proof is not NaN-proof** — `Math.max(0, Math.min(1, NaN))` is `NaN`, so the comment's guarantee ("never … a NaN") is false and `alpha: NaN` would reach `stroke()`. Currently unreachable (`applyStatusEffect` rejects non-finite `expiresAtMs`, `status-effects.ts:13`), so this is a comment-vs-code contradiction rather than a live bug — fix one or the other. [`DungeonScreen.tsx`:335-337]
- [x] [Review][Patch] **`IRON_SKIN_BREATHE_MS` is documented as the "period constant" but is not the period** — `Math.abs(Math.cos(now / 260))` has a period of `π × 260 ≈ 817 ms` with a cusp at each zero crossing. The name and doc mislead anyone tuning the breath. [`ability-vfx.ts`:275]
- [x] [Review][Patch] **The shell-radius derivation comment ignores the second ring** — it justifies r=30 against the badge row's lowest pixel at −32, but the code also strokes at `r + 5 = 35` (and the inner 4 px stroke already reaches 32). Both cross the boundary the comment claims to clear. [`ability-vfx.ts`:269-271, `DungeonScreen.tsx`:341]
- [x] [Review][Patch] **Garbled comment in the fallback branch** — "every class 7.3-7.5 has not reached yet" is not a sentence and does not explain the branch. [`DungeonScreen.tsx`:646]
- [x] [Review][Defer] **Balance constants are hand-transcribed with no drift detection** — the AC5 test asserts durations against a local *copy* of the cooldowns, so a balance change to Avalanche's 1000 ms CD would not fail any test even though it breaks the "at most one instance live" invariant. Same for the hit radii baked into ring sizes. Spec-mandated for now ("never import `game-rules` in host"), but the drift is undetected. [`ability-vfx.test.ts`:13-15] — deferred
- [x] [Review][Defer] **AC1 distinctness rests on colour for the Stone Wall / Iron Skin pair** — both are caster-anchored converging rings of near-identical duration (100→26 vs 120→30); the test's `signatureOf` includes colour, so it passes by construction. Whether dust-brown vs slate-grey separates them at 2–4 m is exactly what the pending manual pass must answer. [`ability-vfx.test.ts`:28-36] — deferred to the Client-UX pass
- [x] [Review][Defer] **VFX handles are never cancelled when their caster vanishes mid-effect** — `engine.add()` returns an id specifically so callers can cancel ("its source entity vanished", `engine.ts:45`), but all three call sites discard it. A Stonehide who is downed, disconnects or leaves in the 140–420 ms after a cast leaves a full-alpha effect playing over an absent or greyed-out body. [`DungeonScreen.tsx`:642, 661, 674] — deferred, cosmetic and sub-half-second
...
- `npx vitest run src/vfx/ability-vfx.test.ts` (in `apps/host-client`) — RED first (module absent: "Failed to load url ./ability-vfx"), then GREEN: **11 passed**.
- `npx vitest run` (in `apps/host-client`) — **35 passed** (7.1's 24 + this story's 11), no regressions in `vfx.test.ts`.
- `npm run typecheck` (repo root, all 10 tsconfigs) — green. One real failure was found and fixed on the way: `exactOptionalPropertyTypes: true` rejected passing `lineWidth: number | undefined` / `filled: boolean | undefined` straight into `RingShockwaveParams`, so `RingSpec.lineWidth` and `RingSpec.filled` are **required** fields in the config table rather than optional (the filled Tremor Stomp under-disc carries `lineWidth: 0`, which the primitive ignores).
- `npm test` (repo root) — **501 passed, 1 failed, 3 skipped**, run twice. Both failures are the documented WSL2 e2e flake, and the two runs failed *differently* at the same two spots: run 1 = `simulation-server did not start within 60s` + `timeout after 5000ms waiting for matching delta`; run 2 = `simulation-server did not start within 60s` + `ability-dispatch.test.ts:227` heal assertion (`expected 50 to be >= 100`). Verified not caused by this story: `grep -rn "host-client" tests/` returns **zero** hits, so no test in `tests/**` exercises any file this story touched. This matches the known-flaky note in Testing Standards and 7.1's identical observation.

### Completion Notes List

**What shipped**

- **First consumer of the 7.1 VFX module.** `DungeonScreen.tsx` contained no `VfxEngine` reference (grepped per Task 1.1), so this story did the one-time wiring: construct after `pixiAppRef.current = app`, `update(Date.now())` once per frame right after `renderFrame(...)`, `clear()` + null before `app.destroy(...)`. Stories 7.3–7.8 must reuse it verbatim, not create a second instance.
- **CLOCK CONTRACT honoured.** The engine is advanced with `Date.now()` — the same clock `renderFrame` (`:99`) and the delta handler use — never `performance.now()` (which the purification-pulse and reward-particle blocks in the same ticker use). A comment at the call site says so, since this was 7.1's worst review finding.
- **`apps/host-client/src/vfx/ability-vfx.ts` (new).** Palette (`STONEHIDE_OCHRE`/`DUST`/`SLATE`), Iron Skin shell constants, the declarative per-ability config table, `getAbilityVfxConfig`, `resolveAbilityVfxPlacement`, `ownsIronSkinShell`. Pure — no PixiJS import, so it is testable without a canvas, and 7.3–7.5 extend it by adding a class key to `CLASS_VFX`.
- **`null` is the fallback contract (AC3).** `getAbilityVfxConfig` returns `null` for every class/index without a bespoke entry, including a `null` class and a malformed/out-of-range/non-integer/NaN index. Only on `null` does the handler run the untouched `entry.flashUntil = Date.now() + ABILITY_FLASH_MS` line, so every other class's `ability:fired` behaviour is mechanically unchanged and `spirit-ability:fired` is not touched at all. For a Stonehide cast `flashUntil` is never set.
- **Mechanical honesty (AC2).** `hitRangePx` is 0 for Stone Wall, Tremor Stomp and Iron Skin (the sim ignores `hitRange` for TAP, and Stone Wall's is 0), 200 for Avalanche; direction is normalized the way `GameRoom.ts:2108-2110` does. A zero-direction Avalanche resolves to `null` and renders **nothing** — the sim skips that hit too (`GameRoom.ts:2203`) — while a zero-direction Tremor Stomp still renders at the caster.
- **Iron Skin shell is snapshot-driven (AC4).** Rendered in `renderFrame` from `player.statusEffects`, never from the delta, so it is correct after reconnect, on late join, and when a batched delta is dropped. Gated on `ownsIronSkinShell(player) && !isDown && !(isSpirit && !isPurified)`, cleaned up by the same create-on-first-seen / prune-on-missing pattern as `statusBadgeGraphics`, mutated in place (`g.clear()` + redraw, no per-frame allocation), fade clamped to `[0,1]` against host/server clock skew, and multiplied by `0.3` while `isFrozen` so the disconnect cue survives.

**Conscious, justified deviations**

1. **Bespoke `Graphics` for the Iron Skin persistent shell** (7.1 AC3 says no bespoke `Graphics` for basic shapes). Justified and pre-authorised by the story (Pitfall 4): the shell is not an effect but a state-driven entity visual with a 3000 ms lifetime; the primitives are fixed-duration one-shots, and expressing a state as one would mean re-triggering per frame — the exact per-frame-allocation anti-pattern. `createTintPulse` is also ruled out because `renderFrame` overwrites `circle.alpha` unconditionally every frame. All four *cast* effects use only `createRingShockwave` / `createBeam` / `createParticleBurst`, with no bespoke shape code.
2. **Shell added with `addChildAt(g, 0)`, not `addChild`.** The story's Task 4.3 placed the block after the badge section "so the shell layers below the badges" — but with `addChild` a later-created object draws *above*, so that ordering alone would not deliver it. The code block sits where instructed; the layering is achieved with the established below-sprites idiom (tethers `:246`, zones `:324`). Geometrically the shell (r=30/35) and the badge row (lowest pixel −32) do not overlap anyway, so this is belt-and-braces for the couch-readability check.
3. **`RingSpec.lineWidth` / `filled` are required, not optional** — forced by `exactOptionalPropertyTypes: true` (see Debug Log).
4. **`AbilityVfxPlacement` carries `normX`/`normY`** beyond the `{casterX, casterY, hitX, hitY}` the story specified. Stone Wall's beam originates at `caster + normDir × 100` (its hit-circle rim) while its `hitRangePx` is 0, so the rim point cannot be derived from `hitX/hitY`. The handler additionally skips a rim-anchored beam when the direction is zero, so it can never draw a zero-length line.

**Volume / budget findings (D-7.1-D, AC5)**

- Longest duration per ability vs its cooldown: Stone Wall 320 < 2000, Tremor Stomp 420 < 4000, Iron Skin 300 < 6000, Avalanche 180 < 1000. **At most one instance per ability per player can be live**, asserted in `ability-vfx.test.ts` so a re-tune cannot silently break it. Avalanche carries **no particle burst** — steady state with 4 Stonehides spamming is 8 concurrent handles (4 × beam+ring), none of them a `Container`.
- **D-7.1-D stays open, deliberately.** 7.2's volume is bounded by construction, so no cap/pool/back-pressure code was added to `VfxEngine`. The likely forcing stories are 7.6 (per-entity status auras) and 7.8 (zone ticks).
- **`latestTransientDelta` batching limitation re-deferred, not fixed.** `App.tsx` was not touched. If the manual pass shows visibly missing casts with 2+ Stonehides, log a new `D-7.2-*` item proposing a delta queue at the `App.tsx` level (Task 6.5). Nothing was logged to `deferred-work.md` yet because that finding can only come from the live pass.

**Hooks triggered**

- **Client-UX hook — TRIGGERED, and the manual pass is NOT YET RUN.** It needs a live host + at least one phone, Stonehide, inside a dungeon (`ability:fired` is dungeon-only), which cannot be driven from this environment. Per the story's own instruction ("do not claim a check passed that was not actually run"), all seven checklist lines are recorded below as **PENDING — user verification required**. This is the one open item on the merge gate.
- **Contract-change hook — not triggered.** No `packages/shared-types/**`, `packages/net-protocol/**`, session lifecycle, reconnect flow, room state, join flow, or prediction/reconciliation surface touched. `host-session.ts` untouched (`ability:fired` was already on the whitelist).
- **Simulation-safety hook — not triggered.** No `apps/simulation-server/**` or `packages/game-rules/**` change; balance numbers are transcribed as local host visual constants and `game-rules` is not imported.
- **Telemetry hook — N/A.** No new user flow, event or KPI.
- **Ownership check — clean.** Every written file is under `apps/host-client/**` (Host Experience Engineer) plus story bookkeeping. No blocked path was touched.

**Client-UX Manual Checklist — results**

| Check | Result |
|---|---|
| Join-flow smoke test (create → QR join → class select → dungeon, no console errors) | PENDING — requires live session |
| Couch readability at 2–4 m, four abilities distinguishable, nothing occluded | PENDING — requires live session |
| Reconnect-state visibility (frozen 0.3 cue + shell dims with it; shell correct after reconnect) | PENDING — requires live session |
| Down/spirit states show no Iron Skin shell | PENDING — requires live session |
| Other classes still produce the old `ABILITY_FLASH_MS` flash | PENDING live — mechanically guaranteed by the `null`-config fallback and covered by `ability-vfx.test.ts` |
| Volume check (hold Avalanche ~30 s, no frame drop, no accumulation) | PENDING live — bounded by construction and asserted in the test file |
| Teardown (no PixiJS "destroyed object" warnings on unmount) | PENDING — requires live session |

**Post-review addendum (2026-07-22).** Three adversarial layers ran (Blind Hunter / Edge Case Hunter / Acceptance Auditor); see *Review Findings* above. Eight patches applied, two spec instructions deliberately overridden with review sanction (Task 3.5's "do not pass `startedAt`" and Task 1.7's "do not restructure the guard" — both were preventing effects from self-expiring when the PixiJS ticker stops, e.g. on a backgrounded host tab). Four items deferred as `D-7.2-A…D`. One correction to this record: **Task 6.4 was wrongly checked** in the bulk task sweep and has been unchecked — the Client-UX manual pass genuinely has not run, exactly as these notes said all along. Typecheck green and 35/35 host-client tests pass after the patches.

**Confidence: 85%** — the pure logic is covered by 11 passing tests and the whole repo typechecks, but no pixel of this has been seen on a real screen: the Client-UX manual pass is the only unverified surface, and it is exactly where a rendering story can still be wrong (draw order, actual legibility at 2–4 m, the shell's breathe/fade feel).

### File List

- `apps/host-client/src/vfx/ability-vfx.ts` (new)
- `apps/host-client/src/vfx/ability-vfx.test.ts` (new)
- `apps/host-client/src/vfx/index.ts` (modified — re-export the new module)
- `apps/host-client/src/screens/DungeonScreen.tsx` (modified — engine wiring, `ability:fired` branch, Iron Skin shell in `renderFrame`, cleanup)
- `_bmad-output/implementation-artifacts/7-2-stonehide-ability-vfx.md` (modified — story bookkeeping)
...
| 2026-07-22 | Implemented — `ability-vfx.ts` config table + placement math, `VfxEngine` wired into `DungeonScreen` (first consumer), four distinct Stonehide cast effects, snapshot-driven Iron Skin shell. 11 new tests; typecheck green; Client-UX manual pass pending. |
| 2026-07-22 | Code review (3 adversarial layers) — 3 decisions resolved, 8 patches applied (`startedAt` stamped at trigger + engine update lifted above the null-state guard, shell layering reverted to `addChild`, engine-null flash fallback, NaN-safe fade clamp, 4 comment/doc corrections), 4 items deferred as D-7.2-A…D. Task 6.4 unchecked — Client-UX manual pass still outstanding. |
