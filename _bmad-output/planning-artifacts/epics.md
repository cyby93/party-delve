...
**Amendment (2026-08-05, create-story):** Story 7.14 was **split into 7.14a (Simulation Engineer) + 7.14b (Host Experience Engineer)** before implementation. The story as written below rests on a premise that a code-level check disproved: it states the hub "already receives and ignores" ability deltas today and gives itself a Non-goal of "no protocol/delta changes." In fact `ability:fired` is broadcast from exactly one site in the entire simulation server — `apps/simulation-server/src/rooms/GameRoom.ts:2436` — and that site sits inside `if (inDungeon)` (`:2434`). No `ability:fired` delta is ever broadcast outside dungeon phase, so wiring `VfxEngine` into `HubWorldScreen.tsx` with no sim change would render nothing at all. (Story 2.8 unblocked hub casts at the *input* layer only; its e2e test asserts a `COOLDOWN_UPDATE`, not a delta.) Making hub casts visible requires editing `GameRoom.ts`, which is outside a host story's ownership and trips the Simulation-safety hook — hence the split, per `CLAUDE.md`'s cross-context rule. **7.14a** ungates the `ability:fired` broadcast and self-scope status effects, deliberately leaving hit-scan damage, projectile spawning, and zone placement dungeon-only (their tick phases are independently dungeon-gated, so spawning them in the hub would strand them). **7.14b** is the extraction and hub wiring below, blocked by 7.14a, with its "trails" acceptance narrowed accordingly — projectile trails and zone visuals do not appear in the hub under 7.14a's boundary. See `7-14a-hub-ability-fired-broadcast.md` and `7-14b-hub-screen-vfx-wiring.md`.
